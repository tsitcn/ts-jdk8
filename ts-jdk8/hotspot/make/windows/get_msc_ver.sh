...
if [ "x$FORCE_MSC_VER" != "x" ]; then
  ...
  # on win7 and vs2010 chinese version, the result is:no file or directory): C/C++ ÓÅ»¯±àÒëÆ÷ 17.00.50727.1 °æ
  CHINESE_TEXT="ÓÅ»¯±àÒëÆ÷"
  if [[ ${MSC_VER_RAW} == *${CHINESE_TEXT}* ]]; then
    MSC_VER_RAW=`cl 2>&1 | "$HEAD" -n 1 | "$SED" 's/.*ÓÅ»¯±àÒëÆ÷[\ ]*\([0-9][0-9.]*\).*/\1/'`
  fi
  # MSC_VER_RAW=17.00.50727.1
