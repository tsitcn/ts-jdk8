...
void LIRGenerator::profile_branch(If* if_instr, If::Condition cond , LIR_Opr left, LIR_Opr right) {
  if (if_instr->should_profile()) {
    ...
    __ cmove_mips(lir_cond(cond), left, right, opr1, opr2, data_offset_reg, as_BasicType(if_instr->x()->type()));
...
#else
        __ branch(lir_cond_equal, src_reg, LIR_OprFact::oopConst(NULL),  Lcont->label());
#endif
      }
...
#ifdef MIPS
    guarantee(false, "not implemented yet for mips");
    // __ cmp();
    // __ cmov();
#else
