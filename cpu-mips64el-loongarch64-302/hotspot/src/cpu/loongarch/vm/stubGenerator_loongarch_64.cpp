...
class StubGenerator: public StubCodeGenerator {
 private:
  ...
  address generate_call_stub(address& return_address) {
    ...
    __ set64(S8, (long)Interpreter::dispatch_table(itos));
  ...
  address generate_get_previous_fp() {
    ...
    address start = __ pc();
    __ enter();
    __ ld_w(V0, old_fp); // callers fp
    __ ld_w(V0, older_fp); // the frame for ps()
    __ leave();
    __ jr(RA);
    return start;
  }
  ...
  address generate_aescrypt_encryptBlock(bool cbc) {
    ...
    __ set64(ftp, (intptr_t)ft_consts);
    ...
    __ bind(tail);
    __ set64(ftp, (intptr_t)fsb_consts);
  ...
  address generate_aescrypt_decryptBlock(bool cbc) {
    ...
    __ set64(rtp, (intptr_t)rt_consts);
    ...
    __ bind(tail);
    __ set64(rtp, (intptr_t)rsb_consts);
  ...
  void generate_sha256_implCompress(const char *name, address &entry, address &entry_mb) {
    ...
    __ set64(kptr, (intptr_t)round_consts);
...
#define __ masm->
  ...
  void generate_initial() {
    ...
    // platform dependent
    StubRoutines::la::_get_previous_fp_entry = generate_get_previous_fp();
  }
