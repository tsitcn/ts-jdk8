...
address MacroAssembler::ic_call(address entry) {
  ...
  patchable_set52(IC_Klass, (long)Universe::non_oop_word());
...
void MacroAssembler::set_last_Java_frame(Register java_thread,
  ...
  // last_java_pc
  if (last_java_pc.is_bound()) {
    jint offs = (target(last_java_pc) - pc()) >> 2;
    guarantee(is_simm(offs, 20), "Not signed 20-bit offset");
    pcaddi(AT, offs);
  } else {
    InstructionMark im(this);
    last_java_pc.add_patch_at(code(), locator());
    pcaddi(AT, 0);
  }
...
void MacroAssembler::tlab_allocate(Register obj, Register var_size_in_bytes, int con_size_in_bytes,
  ...
  blt_far(AT, end, slow_case, false /* unsigned */);
...
void MacroAssembler::eden_allocate(Register obj, Register var_size_in_bytes, int con_size_in_bytes,
  ...
  if (CMSIncrementalMode || !Universe::heap()->supports_inline_contig_alloc()) {
    ...
    blt_far(end, obj, slow_case, false /* unsigned */);
...
void MacroAssembler::tlab_refill(Label& retry, Label& try_eden, Label& slow_case) {
  ...
  if (TLABStats) {
    ...
    ld_w(AT, thread_reg, in_bytes(JavaThread::tlab_slow_allocations_offset()));
    addiu(AT, AT, 1);
    st_w(AT, thread_reg, in_bytes(JavaThread::tlab_slow_allocations_offset()));
  }
  b(try_eden);
  delayed()->nop();
  ...
  if (TLABStats) {
    ...
    ld_w(AT, thread_reg, in_bytes(JavaThread::tlab_number_of_refills_offset()));
    addi_d(AT, AT, 1);
    st_w(AT, thread_reg, in_bytes(JavaThread::tlab_number_of_refills_offset()));
    // accumulate wastage -- t1 is amount free in tlab
    ld_w(AT, thread_reg, in_bytes(JavaThread::tlab_fast_refill_waste_offset()));
    add(AT, AT, t1);
    st_w(AT, thread_reg, in_bytes(JavaThread::tlab_fast_refill_waste_offset()));
  }
  ...
  addi_d(t1, t1, - typeArrayOopDesc::header_size(T_INT));
  addi_d(t1, t1, ThreadLocalAllocBuffer::alignment_reserve());
  shl(t1, log2_intptr(HeapWordSize/sizeof(jint)));
  st_w(t1, top, arrayOopDesc::length_offset_in_bytes());
  ...
  add(top, top, t1);
  addi_d(top, top, - ThreadLocalAllocBuffer::alignment_reserve_in_bytes());
...
void MacroAssembler::li32(Register reg, int imm) {
  if (is_simm12(imm)) {
    ...
    lu12i_w(reg, split_low20(imm >> 12));
    if (split_low12(imm))
      ori(reg, reg, split_low12(imm));
  }
}
...
void MacroAssembler::set64(Register d, jlong value) {
  if (value <= max_jint && value >= min_jint) {
    if (is_simm12(value)) {
      ...
      lu12i_w(d, split_low20(value >> 12));
      if (split_low12(value))
        ori(d, d, split_low12(value));
    }
  } else if (is_simm(value, 52)) {
    lu12i_w(d, split_low20(value >> 12));
    if (split_low12(value))
      ori(d, d, split_low12(value));
    lu32i_d(d, split_low20(value >> 32));
  } else {
    lu12i_w(d, split_low20(value >> 12));
    if (split_low12(value))
      ori(d, d, split_low12(value));
    lu32i_d(d, split_low20(value >> 32));
    lu52i_d(d, d, split_low12(value >> 52));
  }
}
...
void MacroAssembler::patchable_set52(Register d, jlong value) {
  int count = 0;
...
void MacroAssembler::set_narrow_klass(Register dst, Klass* k) {
  ...
  patchable_set52(dst, narrowKlass);
}
...
void MacroAssembler::set_narrow_oop(Register dst, jobject obj) {
  ...
  relocate(rspec, Assembler::narrow_oop_operand);
  patchable_set52(dst, oop_index);
}

void MacroAssembler::li64(Register rd, long imm) {
  // imm64: | imm12_lu52i | imm20_lu32i | imm20_lu12i | imm12_ori |
  lu12i_w(rd, split_low20(imm >> 12));
  if (split_low12(imm))
    ori(rd, rd, split_low12(imm));
  lu32i_d(rd, split_low20(imm >> 32));
  lu52i_d(rd, rd, split_low12(imm >> 52));
}

void MacroAssembler::li52(Register rd, long imm) {
  assert(is_simm(imm, 52), "Not a 52-bit address");
  lu12i_w(rd, split_low20(imm >> 12));
  if (split_low12(imm))
    ori(rd, rd, split_low12(imm));
  lu32i_d(rd, split_low20(imm >> 32));
...
void MacroAssembler::fast_lock(Register objReg, Register boxReg, Register tmpReg, Register scrReg) {
  ...
  if (EmitSync & 1) {
    ...
    if (EmitSync & 2) {
      ...
      sltui(AT, tmpReg, 1); // AT = (tmpReg == 0) ? 1 : 0
      ...
      move(boxReg, tmpReg) ;
      ld_d(tmpReg, Address(tmpReg, ObjectMonitor::owner_offset_in_bytes()-2)) ;
      ...
      //  sync();
      //}
      addi_d(scrReg, boxReg, ObjectMonitor::owner_offset_in_bytes()-2);
      cmpxchg(TREG, Address(scrReg, 0), tmpReg) ;
...
void MacroAssembler::fast_unlock(Register objReg, Register boxReg, Register tmpReg) {
  ...
  if (EmitSync & 4) {
    ...
    if (EmitSync & 8) {
      ...
      Label DONE_LABEL, Stacked, CheckSucc, Inflated, L;
      ...
      ld_d(AT, Address(boxReg, 0)) ;            // Examine the displaced header
      bne(AT, R0, L) ;      // 0 indicates recursive stack-lock
      addi_d(AT, R0, 0x1);
      b(DONE_LABEL);
      bind(L);
      ...
      // It's inflated
      ld_d(boxReg, Address (tmpReg, ObjectMonitor::owner_offset_in_bytes()-2)) ;
      ...
      ld_d(boxReg, Address (tmpReg, ObjectMonitor::cxq_offset_in_bytes()-2)) ;
...
void MacroAssembler::incrementl(Register reg, int value) {
  if (value == min_jint) {
     move(AT, value);
     LP64_ONLY(add_w(reg, reg, AT)) NOT_LP64(addu(reg, reg, AT));
     return;
  }
  ...
  if(Assembler::is_simm16(value)) {
     ...
     LP64_ONLY(move(AT, value); add_w(reg, reg, AT));
  } else {
     move(AT, value);
     LP64_ONLY(add_w(reg, reg, AT)) NOT_LP64(addu(reg, reg, AT));
  }
...
void MacroAssembler::decrementl(Register reg, int value) {
  if (value == min_jint) {
     move(AT, value);
     LP64_ONLY(sub_w(reg, reg, AT)) NOT_LP64(subu(reg, reg, AT));
     return;
  }
  ...
  if (Assembler::is_simm16(value)) {
     NOT_LP64(addiu(reg, reg, -value));
     LP64_ONLY(move(AT, value); sub_w(reg, reg, AT));
  } else {
     move(AT, value);
     LP64_ONLY(sub_w(reg, reg, AT)) NOT_LP64(subu(reg, reg, AT));
  }
...
void MacroAssembler::loadstore(Register reg, Register base, Register disp, int type) {
  switch (type) {
    ...
    case LOAD_BYTE:
      ...
      break;
    case LOAD_U_BYTE:
      ldx_bu(reg, base, disp);
      break;
    case LOAD_SHORT:
      ldx_h(reg, base, disp);
      break;
    case LOAD_U_SHORT:
      ldx_hu(reg, base, disp);
      break;
    case LOAD_INT:
      ldx_w(reg, base, disp);
      break;
    case LOAD_U_INT:
      ldx_wu(reg, base, disp);
      break;
    case LOAD_LONG:
      ldx_d(reg, base, disp);
      break;
...
void MacroAssembler::loadstore(FloatRegister reg, Register base, Register disp, int type) {
  switch (type) {
    case STORE_FLOAT:
      fstx_s(reg, base, disp);
      break;
    case STORE_DOUBLE:
      fstx_d(reg, base, disp);
      break;
    case LOAD_FLOAT:
      fldx_s(reg, base, disp);
      break;
    case LOAD_DOUBLE:
      fldx_d(reg, base, disp);
      break;
     default:
       ShouldNotReachHere();
    }
