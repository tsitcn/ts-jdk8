...
class MacroAssembler: public Assembler {
 ...
 void clear_jweak_tag(Register possibly_jweak);
  ...
  void patchable_set52(Register d, jlong value);

  void dli(Register rd, long imm) { /*li(rd, imm);*/guarantee(0, "LA not implemented yet"); }
  void li64(Register rd, long imm);
  void li52(Register rd, long imm);
  ...
  void loadstore_t(T reg, int base, int index, int scale, int disp, int type) {
    if (index != 0) {
      if ((scale == 0) && (disp == 0)) {
        ...
        add_d(AT, as_Register(base), as_Register(index));
      } else {
        alsl_d(AT, as_Register(index), as_Register(base), scale - 1);
      }
      if (Assembler::is_simm(disp, 12)) {
        loadstore(reg, AT, disp, type);
      } else {
        move(RT4, disp);
        loadstore(reg, AT, RT4, type);
      }
    } else {
      if (Assembler::is_simm(disp, 12)) {
        ...
        move(RT4, disp);
        loadstore(reg, as_Register(base), RT4, type);
      }
    }
  }
