...
void InterpreterMacroAssembler::get_2_byte_integer_at_bcp(Register reg, Register tmp, int offset) {
  ...
  ld_bu(reg, BCP, offset+1);
  ld_bu(tmp, BCP, offset);
  slli_d(reg, reg, 8);
  add_d(reg, tmp, reg);
}
...
void InterpreterMacroAssembler::get_4_byte_integer_at_bcp(Register reg, Register tmp, int offset) {
  ...
  if (offset & 3) { // Offset unaligned?
    ld_bu(reg, BCP, offset+3);
    ld_bu(tmp, BCP, offset+2);
    slli_d(reg, reg, 8);
    add_d(reg, tmp, reg);
...
void InterpreterMacroAssembler::lock_object(Register lock_reg) {
  ...
  if (UseHeavyMonitors) {
    ...
    ld_d(obj_reg, lock_reg, obj_offset);
    ...
    st_d(swap_reg, lock_reg, mark_offset);
...
void InterpreterMacroAssembler::unlock_object(Register lock_reg) {
  ...
  if (UseHeavyMonitors) {
    ...
    addi_d(swap_reg, lock_reg, BasicObjectLock::lock_offset_in_bytes());

    // Load oop into obj_reg(%T1)
    ld_d(obj_reg, lock_reg, BasicObjectLock::obj_offset_in_bytes ());
    //free entry
    ...
    ld_d(header_reg, swap_reg, BasicLock::displaced_header_offset_in_bytes());
    ...
    // Call the runtime routine for slow case.
    st_d(obj_reg, lock_reg, BasicObjectLock::obj_offset_in_bytes()); // restore obj
    call_VM(NOREG,
...
void InterpreterMacroAssembler::increment_mdp_data_at(Register mdp_in,
  ...
  if (decrement) {
    // Decrement the register.
    add_d(AT, mdp_in, reg);
    assert(Assembler::is_simm12(constant), "constant is not a simm12 !");
    ld_d(AT, AT, constant);
...
void InterpreterMacroAssembler::set_mdp_flag_at(Register mdp_in,
  ...
  if(Assembler::is_simm12(header_bits)) {
...
void InterpreterMacroAssembler::update_mdp_by_offset(Register mdp_in,
                                                     int offset_of_disp) {
  assert(ProfileInterpreter, "must be profiling interpreter");
  assert(Assembler::is_simm12(offset_of_disp), "offset is not an simm12");
...
void InterpreterMacroAssembler::update_mdp_by_constant(Register mdp_in,
                                                       int constant) {
  assert(ProfileInterpreter, "must be profiling interpreter");
  if(Assembler::is_simm12(constant)) {
...
void InterpreterMacroAssembler::profile_obj_type(Register obj, const Address& mdo_addr) {
  ...
  if (mdo_addr.index() == noreg) {
    ...
    alsl_d(AT, mdo_addr.index(), mdo_addr.base(), mdo_addr.scale() - 1);
    ld_d(T0, AT, mdo_addr.disp());
  }
...
void InterpreterMacroAssembler::profile_arguments_type(Register mdp, Register callee, Register tmp, bool is_virtual) {
  ...
  if (MethodData::profile_arguments() || MethodData::profile_return()) {
    ...
    if (MethodData::profile_arguments()) {
      ...
      if (Assembler::is_simm12(off_to_args)) {
      ...
      for (int i = 0; i < TypeProfileArgsLimit; i++) {
        if (i > 0 || MethodData::profile_return()) {
          ...
          if (Assembler::is_simm12(-1 * i * TypeStackSlotEntries::per_arg_count())) {
      ...
      if (MethodData::profile_return()) {
        ...
        if (Assembler::is_simm12(-1 * tmp_arg_counts)) {
...
void InterpreterMacroAssembler::profile_return_type(Register mdp, Register ret, Register tmp) {
  ...
  if (ProfileInterpreter && MethodData::profile_return()) {
    ...
    if (MethodData::profile_return_jsr292_only()) {
      ...
      // length
      Label do_profile;
      ld_b(AT, _bcp_register, 0);
      addi_d(AT, AT, -1 * Bytecodes::_invokedynamic);
      beq(AT, R0, do_profile);

      ld_b(AT, _bcp_register, 0);
      addi_d(AT, AT, -1 * Bytecodes::_invokehandle);
      beq(AT, R0, do_profile);
...
void InterpreterMacroAssembler::save_return_value(TosState state, bool is_native_call) {
  if (is_native_call) {
    ...
    st_w(V0, FP, (-9) * wordSize);
    fst_s(F0, FP, (-10) * wordSize);
  } else {
    push(state);
  }
}
...
void InterpreterMacroAssembler::restore_return_value(TosState state, bool is_native_call) {
  if (is_native_call) {
    ...
    ld_w(V0, FP, (-9) * wordSize);
    fld_s(F0, FP, (-10) * wordSize);
  } else {
    pop(state);
  }
}
