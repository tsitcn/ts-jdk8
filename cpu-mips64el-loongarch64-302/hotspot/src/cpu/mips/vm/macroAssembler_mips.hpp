...
class MacroAssembler: public Assembler {
 ...
 void clear_jweak_tag(Register possibly_jweak);
  ...
  void push (Register reg)      { addiu(SP, SP, -4);  sw  (reg, SP, 0); }
  void push (FloatRegister reg) { addiu(SP, SP, -4);  swc1(reg, SP, 0); }
  void pop  (Register reg)      { lw  (reg, SP, 0);  addiu(SP, SP, 4); }
  void pop  (FloatRegister reg) { lwc1(reg, SP, 0);  addiu(SP, SP, 4); }
  void pop  ()                  { addiu(SP, SP, 4); }
  void pop2 ()                  { addiu(SP, SP, 8); }
#endif
...
#else
  void move(Register rd, Register rs)   { addu(rd, rs, R0); }
#endif
