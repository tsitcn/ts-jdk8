...
[
  ...
  if test "x$BUILD_FREETYPE" = xyes; then
    ...
    if test -s "$freetype_lib_path_unix/tsfreetype.dll"; then
      ...
      if test -s "$freetype_lib_path_unix/tsfreetype.lib"; then
...
[
  ...
  if test -s "$POTENTIAL_FREETYPE_INCLUDE_PATH/ft2build.h"; then
    ...
    FREETYPE_LIB_NAME="${LIBRARY_PREFIX}tsfreetype${SHARED_LIBRARY_SUFFIX}"
    if ! test -s "$POTENTIAL_FREETYPE_LIB_PATH/$FREETYPE_LIB_NAME"; then
      ...
      if test "x$OPENJDK_TARGET_OS" = xwindows; then
        ...
        if ! test -s "$POTENTIAL_FREETYPE_LIB_PATH/tsfreetype.lib"; then
          AC_MSG_NOTICE([Could not find $POTENTIAL_FREETYPE_LIB_PATH/tsfreetype.lib. Ignoring location.])
          FOUND_FREETYPE=no
        fi
...
[
  ...
  if test "x$FREETYPE_NOT_NEEDED" = xyes; then
    ...
    if test "x$FREETYPE_LIBS" = x; then
      ...
      if test "x$OPENJDK_TARGET_OS" = xwindows; then
        FREETYPE_LIBS="$FREETYPE_LIB_PATH/tsfreetype.lib"
      else
        FREETYPE_LIBS="-L$FREETYPE_LIB_PATH -ltsfreetype"
      fi
    fi
