...
#include <math.h>
#include "ft2build.h"
#include FT_TS_FREETYPE_H
#include FT_TS_GLYPH_H
#include FT_TS_BBOX_H
#include FT_TS_SIZES_H
#include FT_TS_OUTLINE_H
#include FT_TS_SYNTHESIS_H
#include FT_TS_LCD_FILTER_H
#include FT_TS_MODULE_H

#include "fontscaler.h"

#define  ftFixed1            (FT_TS_Fixed) (1 << 16)
#define  FloatToFTFixed(f)   (FT_TS_Fixed)((f) * (float)(ftFixed1))
...
#define TSJDK_WEIGHT_PLAIN               1.0F
#define TSJDK_WEIGHT_BOLD                2.0F

#define TSJDK_FONT_SHAPE_ENABLE
#define TSJDK_FONT_WEIGHT_ENABLE
#define TSJDK_FONT_ITALIC_ENABLE

#define TSJDK_FREETYPE_LIB               "libtsfreetype.so"
#define TSJDK_FREETYPE_LIB_6             TSJDK_FREETYPE_LIB ".6"
...
typedef struct FTScalerContext {
    FT_TS_Matrix  transform;     /* glyph transform, including device transform */
...
static void freeNativeResources(JNIEnv *env, FTScalerInfo* scalerInfo) {
    ...
    // FT_TS_Done_Face always closes the stream, but only frees the memory
    ...
    FT_TS_Done_Face(scalerInfo->face);
    FT_TS_Done_FreeType(scalerInfo->library);
...
static unsigned long ReadTTFontFileFunc(FT_TS_Stream stream,
...
typedef FT_TS_Error (*FT_TS_Prop_Set_Func)(FT_TS_Library library,
 ...
 */
static void setInterpreterVersion(FT_TS_Library library) {
    ...
    /*
     * FT_TS_Property_Set was introduced in 2.4.11.
    ...
    FT_TS_Property_Set(library, module, property, (void*)(&version));
#else
    void *lib = dlopen(TSJDK_FREETYPE_LIB,   RTLD_LOCAL|RTLD_LAZY);
    if (lib == NULL) {
          lib = dlopen(TSJDK_FREETYPE_LIB_6, RTLD_LOCAL|RTLD_LAZY);
        if (lib == NULL) {
            return;
        }
    }
    FT_TS_Prop_Set_Func func = (FT_TS_Prop_Set_Func)dlsym(lib, "FT_TS_Property_Set");
    if (func != NULL) {
...
Java_sun_font_FreetypeFontScaler_initNativeScaler(
    ...
    */
    error = FT_TS_Init_FreeType(&scalerInfo->library);
    if (error) {
        free(scalerInfo);
        return 0;
    }
    ...
    if (type == TYPE1_FROM_JAVA) { /* TYPE1 */
        ...
        if (scalerInfo->fontData != NULL) {
            ...
            if (bBuffer != NULL) {
                ...
                error = FT_TS_New_Memory_Face(scalerInfo->library,
                                   scalerInfo->fontData,
                                   scalerInfo->fontDataLength,
                                   indexInCollection,
                                   &scalerInfo->face);
        ...
        if (scalerInfo->fontData != NULL) {
            FT_TS_Stream ftstream = (FT_TS_Stream) calloc(1, sizeof(FT_TS_StreamRec));
            if (ftstream != NULL) {
                ...
                if (scalerInfo->directBuffer != NULL) {
                    ...
                    ftstream->pos = 0;
                    ftstream->read = (FT_TS_Stream_IoFunc) ReadTTFontFileFunc;
                    ftstream->close = NULL;
                    ftstream->pathname.pointer = (void *) scalerInfo;

                    memset(&ft_open_args, 0, sizeof(FT_TS_Open_Args));
                    ft_open_args.flags = FT_TS_OPEN_STREAM;
                    ft_open_args.stream = ftstream;

                    error = FT_TS_Open_Face(scalerInfo->library,
                                         &ft_open_args,
                                         indexInCollection,
                                         &scalerInfo->face);
                    if (!error) {
...
Java_sun_font_FreetypeFontScaler_createScalerContextNative(
     ...
     */
    context->doBold    = (boldness_x == TSJDK_WEIGHT_BOLD && boldness_y == TSJDK_WEIGHT_BOLD);
    context->doItalize = (italic   != 0  );
    context->weight_x  =  boldness_x;
    context->weight_y  =  boldness_y;
    context->oblique   =  italic;
    context->flags     =  flags;
     ...
     */
/** I want pass a parameter to control, but have errors. */
#ifdef TSJDK_FONT_SHAPE_ENABLE
    if (bitmapFirst || (aa != TEXT_AA_ON) && (fm != TEXT_FM_ON))
    {
        FT_TS_Matrix* pMatrix = &(context->transform);
        if (FT_TS_GlyphSlot_Is_Valid_BitmapMatrix(pMatrix))
        {
            context->useSbits = 1;
        }
    }
...
#define FT_TS_MATRIX_ONE        0x10000
#define FT_TS_MATRIX_OBLIQUE_XY 0x0366A

static void setupTransform(FT_TS_Matrix* target, FTScalerContext *context) {
    FT_TS_Matrix* transform = &context->transform;
    if (context->doItalize) {
        ...
        target->xx = FT_TS_MATRIX_ONE;
        target->xy = FT_TS_MATRIX_OBLIQUE_XY;
        target->yx = 0;
        target->yy = FT_TS_MATRIX_ONE;
        FT_TS_Matrix_Multiply(transform, target);
    } else {
...
static int setupFTContext(JNIEnv *env,
    ...
    if (context != NULL) {
#ifdef TSJDK_FONT_ITALIC_ENABLE
        FT_TS_Set_Transform(scalerInfo->face, &context->transform, NULL);
#else
        setupTransform(&matrix, context);
        FT_TS_Set_Transform(scalerInfo->face, &matrix, NULL);
#endif

        errCode = FT_TS_Set_Char_Size(scalerInfo->face, 0, context->ptsz, 72, 72);

        if (errCode == 0) {
            errCode = FT_TS_Activate_Size(scalerInfo->face->size);
        }

        FT_TS_Library_SetLcdFilter(scalerInfo->library, FT_TS_LCD_FILTER_DEFAULT);
    }
...
#define OBLIQUE_MODIFIER(y)  (context->doItalize ? ((y)*FT_TS_MATRIX_OBLIQUE_XY/FT_TS_MATRIX_ONE) : 0)
...
#define BOLD_MODIFIER(units_per_EM, y_scale) \
    (context->doBold ? FT_TS_MulFix(units_per_EM, y_scale) / 24 : 0)
...
#define FT_TS_MulFixFloatShift6(a, b) (((float) (a)) * ((float) (b)) / 65536.0 / 64.0)
...
#define contextAwareMetricsY(x, y) \
    ...
    /*
     * See FreeType source code: src/base/ftobjs.c FT_TS_recompute_scaled_metrics()
     * http://icedtea.classpath.org/bugzilla/show_bug.cgi?id=1659
     */
    /* ascent */
    ax = 0;
    ay = -(jfloat) (FT_TS_MulFixFloatShift6(
                       ((jlong) scalerInfo->face->ascender),
                        (jlong) scalerInfo->face->size->metrics.y_scale));
    /* descent */
    dx = 0;
    dy = -(jfloat) (FT_TS_MulFixFloatShift6(
    ...
    /* leading */
    lx = 0;
    ly = (jfloat) (FT_TS_MulFixFloatShift6(
...
/**
 有两处处理粗体斜体的代码，所以统一合并过来。
 所有情形下，都是先粗体再斜体。
 */
static void process_font_bold_italic(FT_TS_GlyphSlot ftglyph, FTScalerContext* context, int renderFlags)
{
    /* apply styles */
#ifdef TSJDK_FONT_WEIGHT_ENABLE
    if (   context->weight_x != TSJDK_WEIGHT_PLAIN
        || context->weight_y != TSJDK_WEIGHT_PLAIN
        || context->doBold) { /* if bold style */
        FT_TS_GlyphSlot_Weight(ftglyph, context->weight_x, context->weight_y, renderFlags, context->flags);
    }
#else
    if (context->doBold) { /* if bold style */
        FT_TS_GlyphSlot_Embolden(ftglyph);
    }
#endif

#ifdef TSJDK_FONT_ITALIC_ENABLE
    if (context->doItalize) { /* if oblique */
        FT_TS_GlyphSlot_Oblique_Direction(ftglyph, context->oblique, context->flags);
    }

    /**
     JDK don't use this anymore.
     see: setupTransform
     FT_TS_GlyphSlot_Oblique(ftglyph);
     */
#endif
}
...
static jlong
    ...
    GlyphInfo *glyphInfo;
    int glyph_index;
    int renderFlags = FT_TS_LOAD_DEFAULT, target;
    FT_TS_GlyphSlot ftglyph;
    ...
    // 据说这样效果更好。
    // renderFlags |= FT_TS_LOAD_DEFAULT | FT_TS_LOAD_RENDER | FT_TS_LOAD_NO_AUTOHINT | FT_TS_OUTLINE_HIGH_PRECISION;
    if (!context->useSbits) {
        renderFlags |= FT_TS_LOAD_NO_BITMAP;
    }
    ...
    if (context->aaType == TEXT_AA_OFF) {
        target = FT_TS_LOAD_TARGET_MONO;
    } else if (context->aaType == TEXT_AA_ON) {
        target = FT_TS_LOAD_TARGET_NORMAL;
    } else if (context->aaType == TEXT_AA_LCD_HRGB ||
               context->aaType == TEXT_AA_LCD_HBGR) {
        target = FT_TS_LOAD_TARGET_LCD;
    } else {
        target = FT_TS_LOAD_TARGET_LCD_V;
    }

    renderFlags |= target;

    glyph_index = FT_TS_Get_Char_Index(scalerInfo->face, glyphCode);

    error = FT_TS_Load_Glyph_Office(scalerInfo->face, glyphCode, renderFlags, context->flags);
    if (error) {
    ...
    process_font_bold_italic(ftglyph, context, renderFlags);
    
    /* generate bitmap if it is not done yet
     e.g. if algorithmic styling is performed and style was added to outline */
    if (renderImage && (ftglyph->format == FT_TS_GLYPH_FORMAT_OUTLINE)) {
        FT_TS_BBox bbox;
        int w, h;
        FT_TS_Outline_Get_CBox(&(ftglyph->outline), &bbox);
        ...
        error = FT_TS_Render_Glyph(ftglyph, FT_TS_LOAD_TARGET_MODE(target));
        if (error != 0) {
            return ptr_to_jlong(getNullGlyphImage());
        }
    }
    ...
    if (renderImage) {
        ...
        if (ftglyph->bitmap.pixel_mode ==  FT_TS_PIXEL_MODE_LCD) {
            glyphInfo->width = width/3;
        } else if (ftglyph->bitmap.pixel_mode ==  FT_TS_PIXEL_MODE_LCD_V) {
    ...
    if (imageSize == 0) {
        ...
        if (ftglyph->bitmap.pixel_mode ==  FT_TS_PIXEL_MODE_MONO) {
            ...
            CopyBW2Grey8(ftglyph->bitmap.buffer,
                         ...
                         width,
                         height);
        } else if (ftglyph->bitmap.pixel_mode ==  FT_TS_PIXEL_MODE_GRAY) {
            /* byte per pixel to byte per pixel => just copy */
            memcpy(glyphInfo->image, ftglyph->bitmap.buffer, imageSize);
        } else if (ftglyph->bitmap.pixel_mode ==  FT_TS_PIXEL_MODE_GRAY4) {
            ...
            CopyGrey4ToGrey8(ftglyph->bitmap.buffer,
                             ...
                             width,
                             height);
        } else if (ftglyph->bitmap.pixel_mode ==  FT_TS_PIXEL_MODE_LCD) {
...
Java_sun_font_FreetypeFontScaler_getGlyphCodeNative(
    ...
    return FT_TS_Get_Char_Index(scalerInfo->face, charCode);
}


#define FloatToF26Dot6(x) ((unsigned int) ((x)*64))

static FT_TS_Outline* getFTOutline(JNIEnv* env, jobject font2D,
    ...
    renderFlags = FT_TS_LOAD_NO_HINTING | FT_TS_LOAD_NO_BITMAP;

    glyph_index = FT_TS_Get_Char_Index(scalerInfo->face, glyphCode);

    error = FT_TS_Load_Glyph(scalerInfo->face, glyphCode, renderFlags);
    if (error) {
        return NULL;
    }
    ...
    FT_TS_Outline_Translate(&ftglyph->outline,
                          FloatToF26Dot6(xpos),
                         -FloatToF26Dot6(ypos));
...
static int lineTo(FT_TS_Vector *to, GPData *gp) {
    addCoords(gp, to);
    addSeg(gp, SEG_LINETO);
    return FT_TS_Err_Ok;
}

static int conicTo(FT_TS_Vector *control, FT_TS_Vector *to, GPData *gp) {
...
static int cubicTo(FT_TS_Vector *control1,
...
static void addToGP(GPData* gpdata, FT_TS_Outline*outline) {
    static const FT_TS_Outline_Funcs outline_funcs = {
        (FT_TS_Outline_MoveToFunc)   moveTo,
        (FT_TS_Outline_LineToFunc)   lineTo,
        (FT_TS_Outline_ConicToFunc) conicTo,
        (FT_TS_Outline_CubicToFunc) cubicTo,
        0, /* shift */
        0, /* delta */
    };

    FT_TS_Outline_Decompose(outline, &outline_funcs, gpdata);
    if (gpdata->numCoords)
        addSeg(gpdata, SEG_CLOSE);

    /* If set to 1, the outline will be filled using the even-odd fill rule */
    if (outline->flags & FT_TS_OUTLINE_EVEN_ODD_FILL) {
...
Java_sun_font_FreetypeFontScaler_getGlyphOutlineBoundsNative(
    ...
    error = FT_TS_Outline_Get_BBox(outline, &bbox);
