...
public class FileFontStrike extends PhysicalStrike {
    ...
    FileFontStrike(FileFont fileFont, FontStrikeDesc desc) {
        ...
        processBoldItalic(fileFont, desc);
        ...
        if (Double.isNaN(matrix[0]) || Double.isNaN(matrix[1]) ||
            ...
            pScalerContext = fileFont.getScaler().createScalerContextXY(matrix,
                                    desc.isBitmapFirst(),
    ...
    private void processBoldItalic(FileFont fileFont, FontStrikeDesc desc)
    {
        if (desc.style != fileFont.style) {
           ...
           */
            if ((desc.style & Font.ITALIC) == Font.ITALIC &&
                ...
                algoStyle = true;
                //italic = 0.7f;
                //italic = Font.POSTURE_OBLIQUE;
            }
            if ((desc.style & Font.BOLD) == Font.BOLD &&
                ((fileFont.style & Font.BOLD) == 0)) {
                algoStyle = true;
                //boldness = 1.33f;
                //boldness = Font.WEIGHT_BOLD;
            }
        }
        if (desc.fontAttrs == null)
        {
            return;
        }
        
        int office_style = 0;
        Object value = desc.fontAttrs.get(TextAttribute.OFFICE_STYLE);
        if (value != null)
        {
            office_style = (Integer)value;
        }

        //check italic
        value = desc.fontAttrs.get(TextAttribute.POSTURE);
        italic = (value != null) ? (Float)value : Font.POSTURE_PLAIN;
        Object temp = desc.fontAttrs.get(TextAttribute.POSTURE_OFFICE);
        if ( (fileFont.style & Font.ITALIC) == Font.ITALIC)
        {
            if (temp == null)
            {
                italic = Font.POSTURE_PLAIN;
            }
            else
            {
                //make italic more italic.
                italic = (Float)temp - italic;
            }
        }
        else if (temp != null)
        {
        ...
        //check bold
        value = desc.fontAttrs.get(TextAttribute.WEIGHT);
        float boldness = (value != null) ? (Float)value : Font.WEIGHT_PLAIN;
        Object tempx = desc.fontAttrs.get(TextAttribute.WEIGHT_X);
        Object tempy = desc.fontAttrs.get(TextAttribute.WEIGHT_Y);
        if ( (fileFont.style & Font.BOLD) == Font.BOLD)
        {
            if (tempx == null)
            {
                boldness_x = Font.WEIGHT_PLAIN;
            }
            else
            {
                //make bold more bold.
                //1 means no change.
                boldness_x = (Float)tempx - boldness + Font.WEIGHT_PLAIN;
            }
            
            if (tempy == null)
            {
                boldness_y = Font.WEIGHT_PLAIN;
            }
            else
            {
                //make bold more bold.
                //1 means no change.
                boldness_y = (Float)tempy - boldness + Font.WEIGHT_PLAIN;
            }
        }
        else
        {
            boldness_x = tempx != null ? (Float)tempx : boldness;
            boldness_y = tempy != null ? (Float)tempy : boldness;
        }
