...
public class PSPrinterJob extends RasterPrinterJob {
 ...
 /* Class Constants */
    ...
    private final static String USR_BIN_PATH ="/usr/bin";
	
    private final static String CMD_LPR      = "lpr";
    private final static String USR_BIN_LPR  = USR_BIN_PATH + "/" + CMD_LPR;

    private final static String CMD_LP       = "lp";
    private final static String USR_BIN_LP   = USR_BIN_PATH + "/" + CMD_LP;
    ...
    private class PrinterSpooler implements java.security.PrivilegedAction {
        ...
        public Object run() {
            ...
            try {
                ...
                // on some os, /usr/bin/lpr has error, then use /usr/bin/lp
                if (0 != result && execCmd[0].endsWith(CMD_LPR))
                {
                    execCmd = printExecCmd(mDestination, mOptions,
                              mNoJobSheet, getJobNameInt(),
                              1, fileName, true);
...
//                        + CURVETO_STR);
   ...
   /**
    ...
    /**
     * on uos, there is no lpr.
     * and deb for app store, forbidden to copy /usr/bin.
     * so I copied to dir of java
     */
    private static String getLprPath()
    {
        if (new File(USR_BIN_LPR).exists())
        {
            return USR_BIN_LPR;
        }
        ...
        String lpr = binPath
                        + File.separator + "cups"
                        + File.separator + CMD_LPR;
        if (new File(lpr).exists())
        {
            return lpr;
        }
        return binPath + File.separator + CMD_LPR;
    }
    ...
    private String[] printExecCmd(String printer, String options,
       ...
       String osname = System.getProperty("os.name");
       if (!forceLP && (osname.equals("Linux") || osname.contains("OS X"))) {
            execCmd = new String[ncomps];
            execCmd[n++] = getLprPath();
            if ((pFlags & PRINTER) != 0) {
                execCmd[n++] = "-P" + printer;
            }
            if ((pFlags & BANNER) != 0) {
