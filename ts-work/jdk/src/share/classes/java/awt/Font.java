...
public class Font implements java.io.Serializable
{
    ...
    public static final float WEIGHT_PLAIN    = TextAttribute.WEIGHT_REGULAR;
    public static final float WEIGHT_BOLD     = TextAttribute.WEIGHT_BOLD;
    
    public static final float POSTURE_PLAIN   = TextAttribute.POSTURE_REGULAR;
    public static final float POSTURE_OBLIQUE = TextAttribute.POSTURE_OBLIQUE;
    ...
    protected boolean postureDirection;
    ...
    private AttributeValues getAttributeValues() {
        if (values == null) {
            ...
            if ((style & BOLD) != 0) {
                valuesTmp.setWeight(WEIGHT_BOLD); // WEIGHT_BOLD
            }

            if ((style & ITALIC) != 0) {
                valuesTmp.setPosture(POSTURE_OBLIQUE); // POSTURE_OBLIQUE
            }
            
            if (postureDirection) {
                valuesTmp.setPostureDirection(postureDirection); // POSTURE_OBLIQUE
            }
    ...
    public Font(String name, int style, int size) {
        this(name, style, size, TextAttribute.POSTURE_DIRECTION_RIGHT);
    }

    public Font(String name, int style, int size, boolean postureDirection) {
        this.name             = (name != null) ? name : "Default";
        this.style            = (style & ~0x03) == 0 ? style : 0;
        this.size             = size;
        this.pointSize        = size;
        this.postureDirection = postureDirection;
    }
    ...
    private Font(AttributeValues values, String oldName, int oldStyle,
        ...
        if (created) {
            ...
            if (oldStyle == -1) {
                ...
                newStyle = checkBoldAndItalic(values);
    ...
    private static int checkBoldAndItalic(AttributeValues values)
    {
        //Be careful. If weight != WEIGHT_BOLD, it means that,
        //even if font file has bold, we dont use it.
        //Italic is same.
        int newStyle = 0;
        if (values.getWeight()  >= WEIGHT_BOLD)     newStyle  = BOLD;
        if (values.getPosture() >= POSTURE_OBLIQUE) newStyle |= ITALIC;
        return newStyle;
    }
     ...
     */
    private static final long RECOGNIZED_MASK = AttributeValues.MASK_ALL
        & ~AttributeValues.getMask(EFONT);

    /**
     * These attributes are considered primary by the FONT attribute.
     */
    private static final long PRIMARY_MASK =
     ...
     */
    private static final long SECONDARY_MASK =
        RECOGNIZED_MASK & ~PRIMARY_MASK;

    /**
     * These attributes are handled by layout.
     */
    private static final long LAYOUT_MASK =
    ...
    private static final long EXTRA_MASK =
    ...
    private void initFromValues(AttributeValues values) {
        ...
        this.style |= checkBoldAndItalic(values);
    ...
    public String toString() {
        ...
        return getClass().getName() + "[family=" + getFamily() + ",name=" + name + ",style=" +
            strStyle + ",size=" + size + ",weight="+(values == null ? "" : values.getWeight())+"]";
    } // toString()
    ...
    public Attribute[] getAvailableAttributes() {
        ...
        Attribute attributes[] = {
            ...
            TextAttribute.OFFICE_STYLE,
            
            TextAttribute.POSTURE_OFFICE,
            TextAttribute.POSTURE_DIRECTION,
            
            TextAttribute.WEIGHT_X,
            TextAttribute.WEIGHT_Y,
            TextAttribute.SOLID_SIZE,
            ...
            TextAttribute.BITMAP_BOLD_GRAY_SORTH,
            TextAttribute.BITMAP_BOLD_GRAY_WEST,
            
            TextAttribute.FLIP_L2R,
            TextAttribute.FLIP_T2B,
    ...
    /**
     * some chars:
     * canDisplay() = false, can draw it.
     * canDisplay() = true,  as a rectangle.
     */
    public int charToGlyph(char c)
    {
        return getFont2D().charToGlyph(c);
    }
   ...
   /**
    ...
    private static void applyStyle(int style, AttributeValues values) {
        ...
        values.setWeight( (style & BOLD)   != 0 ? WEIGHT_BOLD     : WEIGHT_PLAIN);
        // POSTURE_OBLIQUE, POSTURE_REGULAR
        values.setPosture((style & ITALIC) != 0 ? POSTURE_OBLIQUE : POSTURE_PLAIN);
    }
