...
public final class TextLayout implements Cloneable {
    ...
    public Shape getOutline(AffineTransform tx) {
		...
		/*
		// font is transformed, like rotate 90:
		// 1. we can use this code.
		// 2. now we modified PathGraphics.drawString() to solved this problem.
        Shape result;
        LayoutPathImpl lp = textLine.getLayoutPath();
        if (lp != null) {
            result = textLine.getOutline(null);
            result = lp.mapShape(result);
            if (result instanceof GeneralPath)
            {
                ((GeneralPath)result).transform(tx);
            }
        }
        else
        {
            result = textLine.getOutline(tx);
        }
		*/
