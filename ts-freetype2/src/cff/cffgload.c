...
#define FT_TS_COMPONENT  cffgload
                 ...
                 FT_TS_Int32       load_flags )
  {
    ...
    /* check for OT-SVG */
    if ( ( load_flags & FT_TS_LOAD_COLOR )     &&
         ( (TT_Face)glyph->root.face )->svg )
    {
      ...
      error = sfnt->load_svg_doc( (FT_TS_GlyphSlot)glyph, glyph_index );
      if ( !error )
      {
        FT_TS_TRACE3(( "Successfully loaded SVG glyph\n" ));

        glyph->root.format = FT_TS_GLYPH_FORMAT_SVG;
        ...
        advanceX =
          (FT_TS_UShort)FT_TS_MulDiv( advanceX,
                                glyph->root.face->size->metrics.x_ppem,
                                glyph->root.face->units_per_EM );
        advanceY =
          (FT_TS_UShort)FT_TS_MulDiv( advanceY,
