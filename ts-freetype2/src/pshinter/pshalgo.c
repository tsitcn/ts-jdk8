...
#define  COMPUTE_INFLEXS  /* compute inflection points to optimize `S' */
                       ...
                       FT_TS_Memory       memory )
  {
    ...
    /* allocate our tables */
    if ( FT_TS_QNEW_ARRAY( table->sort,  2 * count     ) ||
         FT_TS_QNEW_ARRAY( table->hints,     count     ) ||
         FT_TS_QNEW_ARRAY( table->zones, 2 * count + 1 ) )
      goto Exit;

    table->max_hints   = count;
    table->sort_global = FT_TS_OFFSET( table->sort, count );
    table->num_hints   = 0;
    table->num_zones   = 0;
    table->zone        = NULL;
...
#define  psh_corner_orientation  ft_corner_orientation
                  ...
                  PSH_Globals  globals )
  {
    ...
    if ( FT_TS_QNEW_ARRAY( glyph->points,   outline->n_points   ) ||
         FT_TS_QNEW_ARRAY( glyph->contours, outline->n_contours ) )
      goto Exit;
...
#define  PSH_MAX_STRONG_INTERNAL  16
                                       ...
                                       FT_TS_Int     dimension )
  {
    ...
    else
    {
      ...
      if ( FT_TS_QNEW_ARRAY( strongs, num_strongs ) )
        return;
    }
                  ...
                  FT_TS_Render_Mode  hint_mode )
  {
     ...
     */
    {
      ...
      FT_TS_Fixed  scaled = 0;
      FT_TS_Fixed  fitted = 0;
