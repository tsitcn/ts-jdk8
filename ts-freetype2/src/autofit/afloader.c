...
#define af_intToFixed( i ) \
          ( (FT_TS_Fixed)( (FT_TS_UInt32)(i) << 16 ) )
#define af_fixedToInt( x ) \
          ( (FT_TS_Short)( ( (FT_TS_UInt32)(x) + 0x8000U ) >> 16 ) )
#define af_floatToFixed( f ) \
          ( (FT_TS_Fixed)( (f) * 65536.0 + 0.5 ) )
                                    ...
                                    AF_StyleMetrics  style_metrics )
  {
    ...
    FT_TS_Size_Metrics*  size_metrics = &face->size->internal->autohint_metrics;

    FT_TS_Pos  stdVW = 0;
    FT_TS_Pos  stdHW = 0;

    FT_TS_Bool  size_changed = size_metrics->x_ppem !=
                              globals->stem_darkening_for_ppem;

    FT_TS_Fixed  em_size  = af_intToFixed( face->units_per_EM );

    FT_TS_Matrix  scale_down_matrix = { 0x10000L, 0, 0, 0x10000L };


    /* Skip stem darkening for broken fonts. */
    if ( !face->units_per_EM )
    {
      error = FT_TS_ERR( Corrupted_Font_Header );
      goto Exit;
    }
    ...
    {
      FT_TS_Fixed  darken_by_font_units_x, darken_x;
      ...
      darken_by_font_units_x =
         af_loader_compute_darkening( loader,
                                      face,
                                      stdVW ) ;
      darken_x = FT_TS_MulFix( darken_by_font_units_x,
    ...
    {
      FT_TS_Fixed  darken_by_font_units_y, darken_y;
      ...
      darken_by_font_units_y =
         af_loader_compute_darkening( loader,
                                      face,
                                      stdHW ) ;
      darken_y = FT_TS_MulFix( darken_by_font_units_y,
       ...
       *
       * Reason: `FT_TS_Outline_Embolden' works by shifting the rightmost
      ...
      globals->scale_down_factor =
        FT_TS_DivFix( em_size - ( darken_by_font_units_y + af_intToFixed( 8 ) ),
                   em_size );
    }

    FT_TS_Outline_EmboldenXY( &slot->outline,
                           globals->darken_x,
                           globals->darken_y );

    scale_down_matrix.yy = globals->scale_down_factor;
    FT_TS_Outline_Transform( &slot->outline, &scale_down_matrix );
                        ...
                        FT_TS_Int32   load_flags )
  {
    ...
    FT_TS_Size           size          = face->size;
    FT_TS_Size_Internal  size_internal = size->internal;
    FT_TS_GlyphSlot      slot          = face->glyph;
    FT_TS_Slot_Internal  slot_internal = slot->internal;
    FT_TS_GlyphLoader    gloader       = slot_internal->loader;

    AF_GlyphHints          hints         = loader->hints;
    AF_ScalerRec           scaler;
    AF_StyleMetrics        style_metrics;
    FT_TS_UInt                style_options = AF_STYLE_NONE_DFLT;
    ...
    if ( !size_internal->autohint_metrics.x_scale                          ||
         size_internal->autohint_mode != FT_TS_LOAD_TARGET_MODE( load_flags ) )
    {
      ...
      {
        FT_TS_Size_Metrics*  size_metrics = &size_internal->autohint_metrics;
        ...
        size_metrics->ascender  = FT_TS_PIX_ROUND(
                                    FT_TS_MulFix( face->ascender,
                                               size_metrics->y_scale ) );
        size_metrics->descender = FT_TS_PIX_ROUND(
                                    FT_TS_MulFix( face->descender,
                                               size_metrics->y_scale ) );
        size_metrics->height    = FT_TS_PIX_ROUND(
                                    FT_TS_MulFix( face->height,
                                               size_metrics->y_scale ) );

        size_metrics->x_scale     = FT_TS_DivFix( size_metrics->x_ppem << 6,
                                               face->units_per_EM );
        size_metrics->y_scale     = FT_TS_DivFix( size_metrics->y_ppem << 6,
                                               face->units_per_EM );
        size_metrics->max_advance = FT_TS_PIX_ROUND(
                                      FT_TS_MulFix( face->max_advance_width,
                                                 size_metrics->x_scale ) );
      }
    ...
    scaler.render_mode = FT_TS_LOAD_TARGET_MODE( load_flags );
    scaler.flags       = 0;
     ...
     * FT_TS_GLYPH_FORMAT_OUTLINE by the recursed `FT_TS_Load_Glyph' function.
     * In the rare cases where FT_TS_LOAD_NO_RECURSE is set, it implies
     * FT_TS_LOAD_NO_SCALE and as such the auto-hinter is never called.
     */
    ...
    if ( scaler.render_mode == FT_TS_RENDER_MODE_LIGHT    &&
    ...
    if ( loader->transformed )
    {
      ...
      if ( !FT_TS_Matrix_Invert( &inverse ) )
        FT_TS_Vector_Transform( &loader->trans_delta, &inverse );
    }

    switch ( slot->format )
    {
    case FT_TS_GLYPH_FORMAT_OUTLINE:
      ...
      loader->pp2.x = FT_TS_MulFix( slot->metrics.horiAdvance,
      ...
      if ( scaler.render_mode != FT_TS_RENDER_MODE_LIGHT )
      {
        ...
        if ( axis->num_edges > 1 && AF_HINTS_DO_ADVANCE( hints ) )
        {
          ...
          FT_TS_Pos  old_rsb = loader->pp2.x - edge2->opos;
          /* loader->pp1.x is always zero at this point of time */
          FT_TS_Pos  old_lsb = edge1->opos;     /* - loader->pp1.x */
          FT_TS_Pos  new_lsb = edge1->pos;

          /* remember unhinted values to later account */
          /* for rounding errors                       */
          FT_TS_Pos  pp1x_uh = new_lsb    - old_lsb;
          FT_TS_Pos  pp2x_uh = edge2->pos + old_rsb;
          ...
          loader->pp1.x = FT_TS_PIX_ROUND( pp1x_uh );
          loader->pp2.x = FT_TS_PIX_ROUND( pp2x_uh );
        ...
        {
          FT_TS_Pos  pp1x = loader->pp1.x;
          FT_TS_Pos  pp2x = loader->pp2.x;


          loader->pp1.x = FT_TS_PIX_ROUND( pp1x );
          loader->pp2.x = FT_TS_PIX_ROUND( pp2x );
  ...
  Hint_Metrics:
    {
      FT_TS_BBox    bbox;
      FT_TS_Vector  vvector;


      vvector.x = slot->metrics.vertBearingX - slot->metrics.horiBearingX;
      vvector.y = slot->metrics.vertBearingY - slot->metrics.horiBearingY;
      vvector.x = FT_TS_MulFix( vvector.x, style_metrics->scaler.x_scale );
      vvector.y = FT_TS_MulFix( vvector.y, style_metrics->scaler.y_scale );

      /* transform the hinted outline if needed */
      if ( loader->transformed )
      {
        FT_TS_Outline_Transform( &gloader->base.outline, &loader->trans_matrix );
        FT_TS_Vector_Transform( &vvector, &loader->trans_matrix );
      }

      /* we must translate our final outline by -pp1.x and compute */
      /* the new metrics                                           */
      if ( loader->pp1.x )
        FT_TS_Outline_Translate( &gloader->base.outline, -loader->pp1.x, 0 );

      FT_TS_Outline_Get_CBox( &gloader->base.outline, &bbox );

      bbox.xMin = FT_TS_PIX_FLOOR( bbox.xMin );
      bbox.yMin = FT_TS_PIX_FLOOR( bbox.yMin );
      bbox.xMax = FT_TS_PIX_CEIL(  bbox.xMax );
      bbox.yMax = FT_TS_PIX_CEIL(  bbox.yMax );
      ...
      slot->metrics.vertBearingX = FT_TS_PIX_FLOOR( bbox.xMin + vvector.x );
      slot->metrics.vertBearingY = FT_TS_PIX_FLOOR( bbox.yMax + vvector.y );

      /* for mono-width fonts (like Andale, Courier, etc.) we need */
      /* to keep the original rounded advance width; ditto for     */
      /* digits if all have the same advance width                 */
      if ( scaler.render_mode != FT_TS_RENDER_MODE_LIGHT                       &&
           ( FT_TS_IS_FIXED_WIDTH( slot->face )                              ||
             ( af_face_globals_is_digit( loader->globals, glyph_index ) &&
               style_metrics->digits_have_same_width                    ) ) )
      {
        slot->metrics.horiAdvance =
          FT_TS_MulFix( slot->metrics.horiAdvance,
      ...
      slot->metrics.vertAdvance = FT_TS_MulFix( slot->metrics.vertAdvance,
                                             style_metrics->scaler.y_scale );

      slot->metrics.horiAdvance = FT_TS_PIX_ROUND( slot->metrics.horiAdvance );
      slot->metrics.vertAdvance = FT_TS_PIX_ROUND( slot->metrics.vertAdvance );
                               ...
                               FT_TS_Pos     standard_width )
  {
    ...
    FT_TS_Fixed   ppem, em_ratio;
    FT_TS_Fixed   stem_width, stem_width_per_1000, scaled_stem, darken_amount;
    FT_TS_Int     log_base_2;
    FT_TS_Int     x1, y1, x2, y2, x3, y3, x4, y4;


    ppem         = FT_TS_MAX( af_intToFixed( 4 ),
                           af_intToFixed( face->size->metrics.x_ppem ) );
    units_per_EM = face->units_per_EM;

    em_ratio = FT_TS_DivFix( af_intToFixed( 1000 ),
    ...
    else
    {
      ...
      stem_width_per_1000 = FT_TS_MulFix( stem_width, em_ratio );
    }

    log_base_2 = FT_TS_MSB( (FT_TS_UInt32)stem_width_per_1000 ) +
    ...
    if ( log_base_2 >= 46 )
    {
      ...
      scaled_stem = FT_TS_MulFix( stem_width_per_1000, ppem );

    /* now apply the darkening parameters */
    if ( scaled_stem < af_intToFixed( x1 ) )
      darken_amount = FT_TS_DivFix( af_intToFixed( y1 ), ppem );

    else if ( scaled_stem < af_intToFixed( x2 ) )
    {
      FT_TS_Int  xdelta = x2 - x1;
      FT_TS_Int  ydelta = y2 - y1;
      FT_TS_Int  x      = stem_width_per_1000 -
                       FT_TS_DivFix( af_intToFixed( x1 ), ppem );


      if ( !xdelta )
        goto Try_x3;

      darken_amount = FT_TS_MulDiv( x, ydelta, xdelta ) +
                      FT_TS_DivFix( af_intToFixed( y1 ), ppem );
    }
    ...
    Try_x3:
      {
        ...
        FT_TS_Int  x      = stem_width_per_1000 -
                         FT_TS_DivFix( af_intToFixed( x2 ), ppem );
        ...
        darken_amount = FT_TS_MulDiv( x, ydelta, xdelta ) +
                        FT_TS_DivFix( af_intToFixed( y2 ), ppem );
      }
    }
    ...
    Try_x4:
      {
        ...
        FT_TS_Int  x      = stem_width_per_1000 -
                         FT_TS_DivFix( af_intToFixed( x3 ), ppem );
        ...
        darken_amount = FT_TS_MulDiv( x, ydelta, xdelta ) +
                        FT_TS_DivFix( af_intToFixed( y3 ), ppem );
      }
    }
    ...
    {
    Use_y4:
      darken_amount = FT_TS_DivFix( af_intToFixed( y4 ), ppem );
    }

    /* Convert darken_amount from per 1000 em to true character space. */
    return FT_TS_DivFix( darken_amount, em_ratio );
  }
