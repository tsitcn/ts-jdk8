...
#define COLR_HEADER_SIZE                 14U
  ...
  typedef enum  FT_TS_PaintFormat_Internal_
  {
    FT_TS_COLR_PAINTFORMAT_INTERNAL_SCALE_CENTER         = 18,
    FT_TS_COLR_PAINTFORMAT_INTERNAL_SCALE_UNIFORM        = 20,
    FT_TS_COLR_PAINTFORMAT_INTERNAL_SCALE_UNIFORM_CENTER = 22,
    FT_TS_COLR_PAINTFORMAT_INTERNAL_ROTATE_CENTER        = 26,
    FT_TS_COLR_PAINTFORMAT_INTERNAL_SKEW_CENTER          = 30

  } FT_TS_PaintFormat_Internal;


  typedef struct  BaseGlyphRecord_
  {
    FT_TS_UShort  gid;
    FT_TS_UShort  first_layer_index;
    FT_TS_UShort  num_layers;
  ...
  typedef struct  Colr_
  {
    ...
    FT_TS_UShort  num_base_glyphs;
    FT_TS_UShort  num_layers;

    FT_TS_Byte*  base_glyphs;
    FT_TS_Byte*  layers;

    FT_TS_ULong  num_base_glyphs_v1;
    /* Points at beginning of BaseGlyphV1List. */
    FT_TS_Byte*  base_glyphs_v1;

    FT_TS_ULong  num_layers_v1;
    FT_TS_Byte*  layers_v1;

    FT_TS_Byte*  clip_list;
...
#define FT_TS_COMPONENT  ttcolr
                     ...
                     FT_TS_Stream  stream )
  {
    ...
    FT_TS_ULong  base_glyph_offset, layer_offset;
    FT_TS_ULong  base_glyphs_offset_v1, num_base_glyphs_v1;
    FT_TS_ULong  layer_offset_v1, num_layers_v1, clip_list_offset;
    FT_TS_ULong  table_size;
    ...
    colr->version = FT_TS_NEXT_USHORT( p );
    if ( colr->version != 0 && colr->version != 1 )
      goto InvalidTable;

    colr->num_base_glyphs = FT_TS_NEXT_USHORT( p );
    base_glyph_offset     = FT_TS_NEXT_ULONG( p );
    ...
    layer_offset     = FT_TS_NEXT_ULONG( p );
    colr->num_layers = FT_TS_NEXT_USHORT( p );
    ...
    if ( colr->version == 1 )
    {
      base_glyphs_offset_v1 = FT_TS_NEXT_ULONG( p );

      if ( base_glyphs_offset_v1 >= table_size )
        goto InvalidTable;

      p1                 = (FT_TS_Byte*)( table + base_glyphs_offset_v1 );
      num_base_glyphs_v1 = FT_TS_PEEK_ULONG( p1 );
      ...
      layer_offset_v1 = FT_TS_NEXT_ULONG( p );
      ...
      if ( layer_offset_v1 )
      {
        p1            = (FT_TS_Byte*)( table + layer_offset_v1 );
        num_layers_v1 = FT_TS_PEEK_ULONG( p1 );
      ...
      clip_list_offset = FT_TS_NEXT_ULONG( p );

      if ( clip_list_offset >= table_size )
        goto InvalidTable;

      if ( clip_list_offset )
        colr->clip_list = (FT_TS_Byte*)( table + clip_list_offset );
      else
        colr->clip_list = 0;
    }

    colr->base_glyphs = (FT_TS_Byte*)( table + base_glyph_offset );
    colr->layers      = (FT_TS_Byte*)( table + layer_offset      );
    colr->table       = table;
  ...
  tt_face_free_colr( TT_Face  face )
  {
    ...
    if ( colr )
    {
      FT_TS_FRAME_RELEASE( colr->table );
      FT_TS_FREE( colr );
    }
  }
  ...
  static FT_TS_Bool
  find_base_glyph_record( FT_TS_Byte*          base_glyph_begin,
                          FT_TS_UInt           num_base_glyph,
                          FT_TS_UInt           glyph_id,
                          BaseGlyphRecord*  record )
  {
    FT_TS_UInt  min = 0;
    FT_TS_UInt  max = num_base_glyph;


    while ( min < max )
    {
      FT_TS_UInt   mid = min + ( max - min ) / 2;
      FT_TS_Byte*  p   = base_glyph_begin + mid * BASE_GLYPH_SIZE;

      FT_TS_UShort  gid = FT_TS_NEXT_USHORT( p );
      ...
      {
        record->gid               = gid;
        record->first_layer_index = FT_TS_NEXT_USHORT( p );
        record->num_layers        = FT_TS_NEXT_USHORT( p );
                          ...
                          FT_TS_LayerIterator*  iterator )
  {
    ...
    *aglyph_index = FT_TS_NEXT_USHORT( iterator->p );
    *acolor_index = FT_TS_NEXT_USHORT( iterator->p );

    if ( *aglyph_index >= (FT_TS_UInt)( FT_TS_FACE( face )->num_glyphs )   ||
         ( *acolor_index != 0xFFFF                                 &&
  ...
  static FT_TS_Bool
  read_color_line( FT_TS_Byte*      color_line_p,
                   FT_TS_ColorLine  *colorline )
  {
    FT_TS_Byte*        p = color_line_p;
    FT_TS_PaintExtend  paint_extend;


    paint_extend = (FT_TS_PaintExtend)FT_TS_NEXT_BYTE( p );
    if ( paint_extend > FT_TS_COLR_PAINT_EXTEND_REFLECT )
      return 0;

    colorline->extend = paint_extend;

    colorline->color_stop_iterator.num_color_stops    = FT_TS_NEXT_USHORT( p );
  ...
  /*
   * Read a paint offset for `FT_TS_Paint*` objects that have them and check
  ...
  get_child_table_pointer ( Colr*      colr,
                            FT_TS_Byte*   paint_base,
                            FT_TS_Byte**  p,
                            FT_TS_Byte**  child_table_pointer )
  {
    FT_TS_UInt32  paint_offset;
    FT_TS_Byte*   child_table_p;


    if ( !child_table_pointer )
      return 0;

    paint_offset = FT_TS_NEXT_UOFF3( *p );
    if ( !paint_offset )
      return 0;

    child_table_p = (FT_TS_Byte*)( paint_base + paint_offset );

    if ( child_table_p < colr->paints_start_v1                         ||
         child_table_p >= ( (FT_TS_Byte*)colr->table + colr->table_size ) )
      return 0;
  ...
  read_paint( Colr*           colr,
              FT_TS_Byte*        p,
              FT_TS_COLR_Paint*  apaint )
  {
    FT_TS_Byte*  paint_base     = p;
    FT_TS_Byte*  child_table_p  = NULL;
    ...
    if ( p < colr->paints_start_v1                         ||
         p >= ( (FT_TS_Byte*)colr->table + colr->table_size ) )
      return 0;

    apaint->format = (FT_TS_PaintFormat)FT_TS_NEXT_BYTE( p );

    if ( apaint->format >= FT_TS_COLR_PAINT_FORMAT_MAX )
      return 0;

    if ( apaint->format == FT_TS_COLR_PAINTFORMAT_COLR_LAYERS )
    {
      /* Initialize layer iterator/ */
      FT_TS_Byte    num_layers;
      FT_TS_UInt32  first_layer_index;


      num_layers = FT_TS_NEXT_BYTE( p );
      if ( num_layers > colr->num_layers_v1 )
        return 0;

      first_layer_index = FT_TS_NEXT_ULONG( p );
    ...
    else if ( apaint->format == FT_TS_COLR_PAINTFORMAT_SOLID )
    {
      apaint->u.solid.color.palette_index = FT_TS_NEXT_USHORT( p );
      apaint->u.solid.color.alpha         = FT_TS_NEXT_SHORT( p );

      return 1;
    }

    else if ( apaint->format == FT_TS_COLR_PAINTFORMAT_COLR_GLYPH )
    {
      apaint->u.colr_glyph.glyphID = FT_TS_NEXT_USHORT( p );
    ...
    if ( apaint->format == FT_TS_COLR_PAINTFORMAT_LINEAR_GRADIENT )
    {
      ...
      /*
       * In order to support variations expose these as FT_TS_Fixed 16.16 values so
       * that we can support fractional values after interpolation.
       */
      apaint->u.linear_gradient.p0.x = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.linear_gradient.p0.y = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.linear_gradient.p1.x = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.linear_gradient.p1.y = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.linear_gradient.p2.x = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.linear_gradient.p2.y = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );

      return 1;
    }

    else if ( apaint->format == FT_TS_COLR_PAINTFORMAT_RADIAL_GRADIENT )
    {
      FT_TS_Pos  tmp;
      ...
      apaint->u.radial_gradient.c0.x = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.radial_gradient.c0.y = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );

      tmp                          = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.radial_gradient.r0 = tmp < 0 ? FT_TS_INT_MAX : tmp;

      apaint->u.radial_gradient.c1.x = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.radial_gradient.c1.y = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );

      tmp                          = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.radial_gradient.r1 = tmp < 0 ? FT_TS_INT_MAX : tmp;

      return 1;
    }

    else if ( apaint->format == FT_TS_COLR_PAINTFORMAT_SWEEP_GRADIENT )
    {
      if ( !read_color_line( child_table_p,
    ...
    if ( apaint->format == FT_TS_COLR_PAINTFORMAT_GLYPH )
    {
      apaint->u.glyph.paint.p                     = child_table_p;
      apaint->u.glyph.paint.insert_root_transform = 0;
      apaint->u.glyph.glyphID                     = FT_TS_NEXT_USHORT( p );

      return 1;
    }

    else if ( apaint->format == FT_TS_COLR_PAINTFORMAT_TRANSFORM )
    {
       ...
       */
      apaint->u.transform.affine.xx = FT_TS_NEXT_LONG( p );
      apaint->u.transform.affine.yx = FT_TS_NEXT_LONG( p );
      apaint->u.transform.affine.xy = FT_TS_NEXT_LONG( p );
      apaint->u.transform.affine.yy = FT_TS_NEXT_LONG( p );
      apaint->u.transform.affine.dx = FT_TS_NEXT_LONG( p );
      apaint->u.transform.affine.dy = FT_TS_NEXT_LONG( p );

      return 1;
    }

    else if ( apaint->format == FT_TS_COLR_PAINTFORMAT_TRANSLATE )
    {
      apaint->u.translate.paint.p                     = child_table_p;
      apaint->u.translate.paint.insert_root_transform = 0;

      apaint->u.translate.dx = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.translate.dy = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );

      return 1;
    }

    else if ( apaint->format ==
                FT_TS_COLR_PAINTFORMAT_SCALE                         ||
              (FT_TS_PaintFormat_Internal)apaint->format ==
                FT_TS_COLR_PAINTFORMAT_INTERNAL_SCALE_CENTER         ||
              (FT_TS_PaintFormat_Internal)apaint->format ==
                FT_TS_COLR_PAINTFORMAT_INTERNAL_SCALE_UNIFORM        ||
              (FT_TS_PaintFormat_Internal)apaint->format ==
                FT_TS_COLR_PAINTFORMAT_INTERNAL_SCALE_UNIFORM_CENTER )
    {
      apaint->u.scale.paint.p                     = child_table_p;
      apaint->u.scale.paint.insert_root_transform = 0;

      /* All scale paints get at least one scale value. */
      apaint->u.scale.scale_x = F2DOT14_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      ...
      if ( apaint->format ==
           ...
           (FT_TS_PaintFormat_Internal)apaint->format ==
             FT_TS_COLR_PAINTFORMAT_INTERNAL_SCALE_CENTER )
        apaint->u.scale.scale_y = F2DOT14_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      else
        apaint->u.scale.scale_y = apaint->u.scale.scale_x;

      /* Scale paints that have a center read center coordinates, */
      /* otherwise the center is (0,0).                           */
      if ( (FT_TS_PaintFormat_Internal)apaint->format ==
             FT_TS_COLR_PAINTFORMAT_INTERNAL_SCALE_CENTER         ||
           (FT_TS_PaintFormat_Internal)apaint->format ==
             FT_TS_COLR_PAINTFORMAT_INTERNAL_SCALE_UNIFORM_CENTER )
      {
        apaint->u.scale.center_x = INT_TO_FIXED( FT_TS_NEXT_SHORT ( p ) );
        apaint->u.scale.center_y = INT_TO_FIXED( FT_TS_NEXT_SHORT ( p ) );
      }
      else
      {
      ...
      apaint->format = FT_TS_COLR_PAINTFORMAT_SCALE;

      return 1;
    }

    else if ( apaint->format == FT_TS_COLR_PAINTFORMAT_ROTATE ||
              (FT_TS_PaintFormat_Internal)apaint->format ==
                FT_TS_COLR_PAINTFORMAT_INTERNAL_ROTATE_CENTER )
    {
      apaint->u.rotate.paint.p                     = child_table_p;
      apaint->u.rotate.paint.insert_root_transform = 0;

      apaint->u.rotate.angle = F2DOT14_TO_FIXED( FT_TS_NEXT_SHORT( p ) );

      if ( (FT_TS_PaintFormat_Internal)apaint->format ==
           FT_TS_COLR_PAINTFORMAT_INTERNAL_ROTATE_CENTER )
      {
        apaint->u.rotate.center_x = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
        apaint->u.rotate.center_y = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      }
      else
      {
      ...
      apaint->format = FT_TS_COLR_PAINTFORMAT_ROTATE;

      return 1;
    }

    else if ( apaint->format == FT_TS_COLR_PAINTFORMAT_SKEW ||
              (FT_TS_PaintFormat_Internal)apaint->format ==
                FT_TS_COLR_PAINTFORMAT_INTERNAL_SKEW_CENTER )
    {
      apaint->u.skew.paint.p                     = child_table_p;
      apaint->u.skew.paint.insert_root_transform = 0;

      apaint->u.skew.x_skew_angle = F2DOT14_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      apaint->u.skew.y_skew_angle = F2DOT14_TO_FIXED( FT_TS_NEXT_SHORT( p ) );

      if ( (FT_TS_PaintFormat_Internal)apaint->format ==
           FT_TS_COLR_PAINTFORMAT_INTERNAL_SKEW_CENTER )
      {
        apaint->u.skew.center_x = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
        apaint->u.skew.center_y = INT_TO_FIXED( FT_TS_NEXT_SHORT( p ) );
      }
      else
      {
      ...
      apaint->format = FT_TS_COLR_PAINTFORMAT_SKEW;

      return 1;
    }

    else if ( apaint->format == FT_TS_COLR_PAINTFORMAT_COMPOSITE )
    {
      FT_TS_UInt  composite_mode;


      apaint->u.composite.source_paint.p                     = child_table_p;
      apaint->u.composite.source_paint.insert_root_transform = 0;

      composite_mode = FT_TS_NEXT_BYTE( p );
      if ( composite_mode >= FT_TS_COLR_COMPOSITE_MAX )
        return 0;

      apaint->u.composite.composite_mode = (FT_TS_Composite_Mode)composite_mode;
  ...
  static FT_TS_Bool
  find_base_glyph_v1_record( FT_TS_Byte *           base_glyph_begin,
                             FT_TS_UInt             num_base_glyph,
                             FT_TS_UInt             glyph_id,
                             BaseGlyphV1Record  *record )
  {
    ...
    while ( min < max )
    {
       ...
       */
      FT_TS_Byte  *p = base_glyph_begin + 4 + mid * BASE_GLYPH_PAINT_RECORD_SIZE;
      ...
      {
        record->gid          = gid;
        record->paint_offset = FT_TS_NEXT_ULONG ( p );
        return 1;
      }
    }
                                ...
                                FT_TS_OpaquePaint*          opaque_paint )
  {
    ...
    p = (FT_TS_Byte*)( colr->base_glyphs_v1 +
                    base_glyph_v1_record.paint_offset );
    if ( p >= ( (FT_TS_Byte*)colr->table + colr->table_size ) )
      return 0;

    opaque_paint->p = p;

    if ( root_transform == FT_TS_COLOR_INCLUDE_ROOT_TRANSFORM )
                                   ...
                                   FT_TS_ClipBox*  clip_box )
  {
    ...
    FT_TS_Byte  *p, *p1, *clip_base, *limit;

    FT_TS_Byte    clip_list_format;
    FT_TS_ULong   num_clip_boxes, i;
    FT_TS_UShort  gid_start, gid_end;
    FT_TS_UInt32  clip_box_offset;
    FT_TS_Byte    format;

    const FT_TS_Byte  num_corners = 4;
    FT_TS_Vector      corners[4];
    FT_TS_Byte        j;
    FT_TS_BBox        font_clip_box;
    ...
    limit = (FT_TS_Byte*)colr->table + colr->table_size;
    ...
    clip_base        = p;
    clip_list_format = FT_TS_NEXT_BYTE ( p );
    ...
    num_clip_boxes = FT_TS_NEXT_ULONG( p );
    ...
    {
      gid_start       = FT_TS_NEXT_USHORT( p );
      gid_end         = FT_TS_NEXT_USHORT( p );
      clip_box_offset = FT_TS_NEXT_UOFF3( p );

      if ( base_glyph >= gid_start && base_glyph <= gid_end )
      {
        p1 = (FT_TS_Byte*)( clip_base + clip_box_offset );

        /* Check whether we can extract one `uint8`. */
        if ( p1 > limit - 1 )
          return 0;

        format = FT_TS_NEXT_BYTE( p1 );
        ...
        font_clip_box.xMin = FT_TS_MulFix( FT_TS_NEXT_SHORT( p1 ),
                                        face->root.size->metrics.x_scale );
        font_clip_box.yMin = FT_TS_MulFix( FT_TS_NEXT_SHORT( p1 ),
                                        face->root.size->metrics.x_scale );
        font_clip_box.xMax = FT_TS_MulFix( FT_TS_NEXT_SHORT( p1 ),
                                        face->root.size->metrics.x_scale );
        font_clip_box.yMax = FT_TS_MulFix( FT_TS_NEXT_SHORT( p1 ),
                                        face->root.size->metrics.x_scale );
        ...
        for ( j = 0; j < num_corners; ++j )
        {
          if ( face->root.internal->transform_flags & 1 )
            FT_TS_Vector_Transform( &corners[j],
  ...
  {
    FT_TS_Byte*   p             = NULL;
    FT_TS_Byte*   p_first_layer = NULL;
    FT_TS_Byte*   p_paint       = NULL;
    FT_TS_UInt32  paint_offset;
    ...
    p_first_layer = p -
                      ...
                      LAYER_V1_LIST_NUM_LAYERS_SIZE;
    if ( p_first_layer < (FT_TS_Byte*)colr->layers_v1 )
      return 0;
    if ( p_first_layer >= (FT_TS_Byte*)(
    ...
    p_paint = (FT_TS_Byte*)( colr->layers_v1 + paint_offset );

    if ( p_paint < colr->paints_start_v1                         ||
         p_paint >= ( (FT_TS_Byte*)colr->table + colr->table_size ) )
      return 0;
                               ...
                               FT_TS_ColorStopIterator  *iterator )
  {
    ...
    if ( iterator->p +
           ( ( iterator->num_color_stops - iterator->current_color_stop ) *
             COLOR_STOP_SIZE ) >
         ( (FT_TS_Byte *)colr->table + colr->table_size ) )
      return 0;

    /* Iterator points at first `ColorStop` of `ColorLine`. */
    p = iterator->p;

    color_stop->stop_offset = FT_TS_NEXT_SHORT( p );

    color_stop->color.palette_index = FT_TS_NEXT_USHORT( p );

    color_stop->color.alpha = FT_TS_NEXT_SHORT( p );
  ...
  {
    Colr*           colr = (Colr*)face->colr;
    FT_TS_OpaquePaint  next_paint;
    FT_TS_Matrix       ft_root_scale;
    ...
    if ( opaque_paint.insert_root_transform )
    {
       ...
       * i.e., `FT_TS_Size` is not applied or multiplied into the values.  When
       ...
       * transform and translate configured through `FT_TS_Set_Transform`.
       ...
       * transform here.
       */
      paint->format = FT_TS_COLR_PAINTFORMAT_TRANSFORM;
      ...
      if ( face->root.internal->transform_flags & 1 )
        FT_TS_Matrix_Multiply( &face->root.internal->transform_matrix,
                            &ft_root_scale );
       ...
       * documentation of `FT_TS_Set_Translate`, is performed on the character
       * size given in the last call to `FT_TS_Set_Char_Size`.  The
       * 'PaintTransform' paint table's `FT_TS_Affine23` format expects
                            ...
                            FT_TS_GlyphSlot  srcSlot )
  {
    ...
    if ( !dstSlot->bitmap.buffer )
    {
      ...
      dstSlot->bitmap.pixel_mode = FT_TS_PIXEL_MODE_BGRA;
      ...
      FT_TS_MEM_ZERO( dstSlot->bitmap.buffer, size );
    }
    else
    {
      /* Resize destination if needed such that new component fits. */
      FT_TS_Int  x_min, x_max, y_min, y_max;


      x_min = FT_TS_MIN( dstSlot->bitmap_left, srcSlot->bitmap_left );
      x_max = FT_TS_MAX( dstSlot->bitmap_left + (FT_TS_Int)dstSlot->bitmap.width,
                      srcSlot->bitmap_left + (FT_TS_Int)srcSlot->bitmap.width );

      y_min = FT_TS_MIN( dstSlot->bitmap_top - (FT_TS_Int)dstSlot->bitmap.rows,
                      srcSlot->bitmap_top - (FT_TS_Int)srcSlot->bitmap.rows );
      y_max = FT_TS_MAX( dstSlot->bitmap_top, srcSlot->bitmap_top );

      if ( x_min != dstSlot->bitmap_left                                 ||
           x_max != dstSlot->bitmap_left + (FT_TS_Int)dstSlot->bitmap.width ||
           y_min != dstSlot->bitmap_top - (FT_TS_Int)dstSlot->bitmap.rows   ||
           y_max != dstSlot->bitmap_top                                  )
      {
        FT_TS_Memory  memory = face->root.memory;

        FT_TS_UInt  width = (FT_TS_UInt)( x_max - x_min );
        FT_TS_UInt  rows  = (FT_TS_UInt)( y_max - y_min );
        FT_TS_UInt  pitch = width * 4;
        ...
        {
          FT_TS_MEM_COPY( q, p, dstSlot->bitmap.width * 4 );
        ...
        dstSlot->internal->flags |= FT_TS_GLYPH_OWN_BITMAP;
        dstSlot->format           = FT_TS_GLYPH_FORMAT_BITMAP;
      }
    }
    ...
    for ( y = 0; y < srcSlot->bitmap.rows; y++ )
    {
      for ( x = 0; x < srcSlot->bitmap.width; x++ )
      {
        ...
        dst[4 * x + 0] = (FT_TS_Byte)( bb * ba2 / 255 + fb );
        dst[4 * x + 1] = (FT_TS_Byte)( bg * ba2 / 255 + fg );
        dst[4 * x + 2] = (FT_TS_Byte)( br * ba2 / 255 + fr );
        dst[4 * x + 3] = (FT_TS_Byte)( ba * ba2 / 255 + fa );
      }
