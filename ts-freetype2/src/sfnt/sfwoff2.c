...
#define FT_TS_COMPONENT  sfwoff2


#define READ_255USHORT( var )  FT_TS_SET_ERROR( Read255UShort( stream, &var ) )

#define READ_BASE128( var )    FT_TS_SET_ERROR( ReadBase128( stream, &var ) )

  /* `var' should be FT_TS_ULong */
...
#define HAVE_OVERLAP_SIMPLE_BITMAP  0x1
  ...
  static void
  stream_close( FT_TS_Stream  stream )
  {
                ...
                const void*  b )
  {
    ...
    FT_TS_Tag  tag1 = table1->Tag;
    FT_TS_Tag  tag2 = table2->Tag;
  ...
  static FT_TS_Error
  Read255UShort( FT_TS_Stream   stream,
                 FT_TS_UShort*  value )
  {
    const FT_TS_Byte    oneMoreByteCode1 = 255;
    const FT_TS_Byte    oneMoreByteCode2 = 254;
    const FT_TS_Byte    wordCode         = 253;
    const FT_TS_UShort  lowestUCode      = 253;

    FT_TS_Error   error        = FT_TS_Err_Ok;
    FT_TS_Byte    code;
    FT_TS_Byte    result_byte  = 0;
    FT_TS_UShort  result_short = 0;


    if ( FT_TS_READ_BYTE( code ) )
      return error;
    if ( code == wordCode )
    {
      /* Read next two bytes and store `FT_TS_UShort' value. */
      if ( FT_TS_READ_USHORT( result_short ) )
        return error;
      *value = result_short;
      return FT_TS_Err_Ok;
    }
    ...
    {
      if ( FT_TS_READ_BYTE( result_byte ) )
        return error;
  ...
  static FT_TS_Error
  ReadBase128( FT_TS_Stream  stream,
               FT_TS_ULong*  value )
  {
  ...
  static FT_TS_Error
  write_buf( FT_TS_Byte**  dst_bytes,
             FT_TS_ULong*  dst_size,
             FT_TS_ULong*  offset,
             FT_TS_Byte*   src,
             FT_TS_ULong   size,
             FT_TS_Memory  memory )
  {
    ...
    {
      FT_TS_TRACE6(( "Reallocating %lu to %lu.\n",
                  *dst_size, (*offset + size) ));
      if ( FT_TS_REALLOC( dst,
                       (FT_TS_ULong)( *dst_size ),
                       (FT_TS_ULong)( *offset + size ) ) )
        goto Exit;
  ...
  static FT_TS_Error
  pad4( FT_TS_Byte**  sfnt_bytes,
        FT_TS_ULong*  sfnt_size,
        FT_TS_ULong*  out_offset,
        FT_TS_Memory  memory )
  {
    FT_TS_Byte*  sfnt        = *sfnt_bytes;
    FT_TS_ULong  dest_offset = *out_offset;

    FT_TS_Byte   zeroes[] = { 0, 0, 0 };
    FT_TS_ULong  pad_bytes;
  ...
  static FT_TS_ULong
  compute_ULong_sum( FT_TS_Byte*  buf,
                     FT_TS_ULong  size )
  {
    FT_TS_ULong  checksum     = 0;
    FT_TS_ULong  aligned_size = size & ~3UL;
    FT_TS_ULong  i;
    FT_TS_ULong  v;


    for ( i = 0; i < aligned_size; i += 4 )
      checksum += ( (FT_TS_ULong)buf[i    ] << 24 ) |
                  ( (FT_TS_ULong)buf[i + 1] << 16 ) |
                  ( (FT_TS_ULong)buf[i + 2] <<  8 ) |
                  ( (FT_TS_ULong)buf[i + 3] <<  0 );
  ...
  static FT_TS_Error
  woff2_decompress( FT_TS_Byte*        dst,
                    FT_TS_ULong        dst_size,
                    const FT_TS_Byte*  src,
                    FT_TS_ULong        src_size )
  {
    /* this cast is only of importance on 32bit systems; */
    /* we don't validate it                              */
    FT_TS_Offset            uncompressed_size = (FT_TS_Offset)dst_size;
    ...
    {
      FT_TS_ERROR(( "woff2_decompress: Stream length mismatch.\n" ));
      return FT_TS_THROW( Invalid_Table );
    }

    FT_TS_TRACE2(( "woff2_decompress: Brotli stream decompressed.\n" ));
    return FT_TS_Err_Ok;
  }
  ...
  static FT_TS_Error
  read_num_hmetrics( FT_TS_Stream   stream,
                     FT_TS_UShort*  num_hmetrics )
  {
    FT_TS_Error   error = FT_TS_Err_Ok;
    FT_TS_UShort  num_metrics;


    if ( FT_TS_STREAM_SKIP( 34 )  )
      return FT_TS_THROW( Invalid_Table );

    if ( FT_TS_READ_USHORT( num_metrics ) )
      return FT_TS_THROW( Invalid_Table );
  ...
  /* An auxiliary function for overflow-safe addition. */
  static FT_TS_Int
  safe_int_addition( FT_TS_Int   a,
                     FT_TS_Int   b,
                     FT_TS_Int*  result )
  {
    if ( ( ( a > 0 ) && ( b > FT_TS_INT_MAX - a ) ) ||
         ( ( a < 0 ) && ( b < FT_TS_INT_MIN - a ) ) )
      return FT_TS_THROW( Invalid_Table );
   ...
   */
  static FT_TS_Error
  triplet_decode( const FT_TS_Byte*  flags_in,
                  const FT_TS_Byte*  in,
                  FT_TS_ULong        in_size,
                  FT_TS_ULong        n_points,
                  WOFF2_Point     result,
                  FT_TS_ULong*       in_bytes_used )
  {
    FT_TS_Int  x = 0;
    FT_TS_Int  y = 0;
    FT_TS_Int  dx;
    ...
    FT_TS_ULong  triplet_index = 0;
    FT_TS_ULong  data_bytes;
    ...
    {
      FT_TS_Byte  flag     = flags_in[i];
      FT_TS_Bool  on_curve = !( flag >> 7 );
  ...
  static FT_TS_Error
  store_points( FT_TS_ULong           n_points,
                const WOFF2_Point  points,
                FT_TS_UShort          n_contours,
                FT_TS_UShort          instruction_len,
                FT_TS_Bool            have_overlap,
                FT_TS_Byte*           dst,
                FT_TS_ULong           dst_size,
                FT_TS_ULong*          glyph_size )
  {
    FT_TS_UInt   flag_offset  = 10 + ( 2 * n_contours ) + 2 + instruction_len;
    FT_TS_Byte   last_flag    = 0xFFU;
    FT_TS_Byte   repeat_count = 0;
    FT_TS_Int    last_x       = 0;
    FT_TS_Int    last_y       = 0;
    FT_TS_UInt   x_bytes      = 0;
    FT_TS_UInt   y_bytes      = 0;
    FT_TS_UInt   xy_bytes;
    FT_TS_UInt   i;
    FT_TS_UInt   x_offset;
    FT_TS_UInt   y_offset;
    ...
    for ( i = 0; i < n_points; ++i )
    {
      ...
      FT_TS_Byte  flag = point.on_curve ? GLYF_ON_CURVE : 0;
      FT_TS_Int   dx   = point.x - last_x;
      FT_TS_Int   dy   = point.y - last_y;
    ...
    for ( i = 0; i < n_points; ++i )
    {
      FT_TS_Int  dx = points[i].x - last_x;
      FT_TS_Int  dy = points[i].y - last_y;


      if ( dx == 0 )
        ;
      else if ( dx > -256 && dx < 256 )
        dst[x_offset++] = (FT_TS_Byte)FT_TS_ABS( dx );
      else
      {
      ...
      if ( dy == 0 )
        ...
        dst[y_offset++] = (FT_TS_Byte)FT_TS_ABS( dy );
      else
      {
  ...
  static void
  compute_bbox( FT_TS_ULong           n_points,
                const WOFF2_Point  points,
                FT_TS_Byte*           dst,
                FT_TS_UShort*         src_x_min )
  {
    FT_TS_Int  x_min = 0;
    FT_TS_Int  y_min = 0;
    FT_TS_Int  x_max = 0;
    ...
    {
      FT_TS_Int  x = points[i].x;
      FT_TS_Int  y = points[i].y;


      x_min = FT_TS_MIN( x, x_min );
      y_min = FT_TS_MIN( y, y_min );
      x_max = FT_TS_MAX( x, x_max );
      y_max = FT_TS_MAX( y, y_max );
    }
    ...
    *src_x_min = (FT_TS_UShort)x_min;
  }


  static FT_TS_Error
  compositeGlyph_size( FT_TS_Stream  stream,
                       FT_TS_ULong   offset,
                       FT_TS_ULong*  size,
                       FT_TS_Bool*   have_instructions )
  {
    FT_TS_Error   error        = FT_TS_Err_Ok;
    FT_TS_ULong   start_offset = offset;
    FT_TS_Bool    we_have_inst = FALSE;
    FT_TS_UShort  flags        = FLAG_MORE_COMPONENTS;


    if ( FT_TS_STREAM_SEEK( start_offset ) )
      goto Exit;
    while ( flags & FLAG_MORE_COMPONENTS )
    {
      FT_TS_ULong  arg_size;
      ...
      if ( FT_TS_STREAM_SKIP( arg_size ) )
        goto Exit;
    }

    *size              = FT_TS_STREAM_POS() - start_offset;
  ...
  static FT_TS_Error
  store_loca( FT_TS_ULong*  loca_values,
              FT_TS_ULong   loca_values_size,
              FT_TS_UShort  index_format,
              FT_TS_ULong*  checksum,
              FT_TS_Byte**  sfnt_bytes,
              FT_TS_ULong*  sfnt_size,
              FT_TS_ULong*  out_offset,
              FT_TS_Memory  memory )
  {
    ...
    FT_TS_Byte*  loca_buf = NULL;
    FT_TS_Byte*  dst      = NULL;

    FT_TS_UInt   i = 0;
    FT_TS_ULong  loca_buf_size;

    const FT_TS_ULong  offset_size = index_format ? 4 : 2;
    ...
    if ( FT_TS_QNEW_ARRAY( loca_buf, loca_buf_size ) )
      goto Fail;

    dst = loca_buf;
    for ( i = 0; i < loca_values_size; i++ )
    {
      FT_TS_ULong  value = loca_values[i];
  ...
  static FT_TS_Error
  reconstruct_glyf( FT_TS_Stream    stream,
                    FT_TS_ULong*    glyf_checksum,
                    FT_TS_ULong*    loca_checksum,
                    FT_TS_Byte**    sfnt_bytes,
                    FT_TS_ULong*    sfnt_size,
                    FT_TS_ULong*    out_offset,
                    WOFF2_Info   info,
                    FT_TS_Memory    memory )
  {
    ...
    const FT_TS_ULong  pos = FT_TS_STREAM_POS();

    FT_TS_UInt  num_substreams = 7;

    FT_TS_UShort  option_flags;
    FT_TS_UShort  num_glyphs;
    FT_TS_UShort  index_format;
    FT_TS_ULong   expected_loca_length;
    FT_TS_UInt    offset;
    FT_TS_UInt    i;
    FT_TS_ULong   points_size;
    FT_TS_ULong   glyph_buf_size;
    FT_TS_ULong   bbox_bitmap_offset;
    FT_TS_ULong   bbox_bitmap_length;
    FT_TS_ULong   overlap_bitmap_offset = 0;
    FT_TS_ULong   overlap_bitmap_length = 0;

    const FT_TS_ULong  glyf_start  = *out_offset;
    FT_TS_ULong        dest_offset = *out_offset;

    WOFF2_Substream  substreams = NULL;

    FT_TS_ULong*    loca_values  = NULL;
    FT_TS_UShort*   n_points_arr = NULL;
    FT_TS_Byte*     glyph_buf    = NULL;
    WOFF2_Point  points       = NULL;


    if ( FT_TS_NEW_ARRAY( substreams, num_substreams ) )
      goto Fail;

    if ( FT_TS_STREAM_SKIP( 2 ) )
      goto Fail;
    if ( FT_TS_READ_USHORT( option_flags ) )
      goto Fail;
    if ( FT_TS_READ_USHORT( num_glyphs ) )
      goto Fail;
    if ( FT_TS_READ_USHORT( index_format ) )
      goto Fail;

    FT_TS_TRACE4(( "option_flags = %u; num_glyphs = %u; index_format = %u\n",
    ...
    {
      FT_TS_ULong  substream_size;


      if ( FT_TS_READ_ULONG( substream_size ) )
        goto Fail;
      ...
      FT_TS_TRACE5(( "  Substream %d: offset = %lu; size = %lu;\n",
    ...
    if ( option_flags & HAVE_OVERLAP_SIMPLE_BITMAP )
    {
      ...
      FT_TS_TRACE5(( "  Overlap bitmap: offset = %lu; size = %lu;\n",
                  overlap_bitmap_offset, overlap_bitmap_length ));
      offset += overlap_bitmap_length;
    }

    if ( FT_TS_NEW_ARRAY( loca_values, num_glyphs + 1 ) )
      goto Fail;
    ...
    if ( FT_TS_NEW_ARRAY( glyph_buf, glyph_buf_size ) )
      goto Fail;

    if ( FT_TS_NEW_ARRAY( info->x_mins, num_glyphs ) )
      goto Fail;

    for ( i = 0; i < num_glyphs; ++i )
    {
      FT_TS_ULong   glyph_size = 0;
      FT_TS_UShort  n_contours = 0;
      FT_TS_Bool    have_bbox  = FALSE;
      FT_TS_Byte    bbox_bitmap;
      FT_TS_ULong   bbox_offset;
      FT_TS_UShort  x_min      = 0;


      /* Set `have_bbox'. */
      bbox_offset = bbox_bitmap_offset + ( i >> 3 );
      if ( FT_TS_STREAM_SEEK( bbox_offset ) ||
           FT_TS_READ_BYTE( bbox_bitmap )   )
        goto Fail;
      if ( bbox_bitmap & ( 0x80 >> ( i & 7 ) ) )
        have_bbox = TRUE;

      /* Read value from `nContourStream'. */
      if ( FT_TS_STREAM_SEEK( substreams[N_CONTOUR_STREAM].offset ) ||
           FT_TS_READ_USHORT( n_contours )                          )
        goto Fail;
      ...
      {
        /* composite glyph */
        FT_TS_Bool    have_instructions = FALSE;
        FT_TS_UShort  instruction_size  = 0;
        FT_TS_ULong   composite_size    = 0;
        FT_TS_ULong   size_needed;
        FT_TS_Byte*   pointer           = NULL;
        ...
        {
          if ( FT_TS_STREAM_SEEK( substreams[GLYPH_STREAM].offset ) ||
               READ_255USHORT( instruction_size )                )
            goto Fail;
          substreams[GLYPH_STREAM].offset = FT_TS_STREAM_POS();
        }

        size_needed = 12 + composite_size + instruction_size;
        if ( glyph_buf_size < size_needed )
        {
          if ( FT_TS_RENEW_ARRAY( glyph_buf, glyph_buf_size, size_needed ) )
            goto Fail;
        ...
        if ( FT_TS_STREAM_SEEK( substreams[BBOX_STREAM].offset ) ||
             FT_TS_READ_USHORT( x_min )                          )
          goto Fail;
        /* No increment here because we read again. */

        if ( FT_TS_STREAM_SEEK( substreams[BBOX_STREAM].offset ) ||
             FT_TS_STREAM_READ( glyph_buf + glyph_size, 8 )      )
          goto Fail;

        substreams[BBOX_STREAM].offset += 8;
        glyph_size                     += 8;

        if ( FT_TS_STREAM_SEEK( substreams[COMPOSITE_STREAM].offset )    ||
             FT_TS_STREAM_READ( glyph_buf + glyph_size, composite_size ) )
          goto Fail;
        ...
        if ( have_instructions )
        {
          ...
          if ( FT_TS_STREAM_SEEK( substreams[INSTRUCTION_STREAM].offset )    ||
               FT_TS_STREAM_READ( glyph_buf + glyph_size, instruction_size ) )
            goto Fail;
      ...
      {
        /* simple glyph */
        FT_TS_ULong   total_n_points = 0;
        FT_TS_UShort  n_points_contour;
        FT_TS_UInt    j;
        FT_TS_ULong   flag_size;
        FT_TS_ULong   triplet_size;
        FT_TS_ULong   triplet_bytes_used;
        FT_TS_Bool    have_overlap  = FALSE;
        FT_TS_Byte    overlap_bitmap;
        FT_TS_ULong   overlap_offset;
        FT_TS_Byte*   flags_buf     = NULL;
        FT_TS_Byte*   triplet_buf   = NULL;
        FT_TS_UShort  instruction_size;
        FT_TS_ULong   size_needed;
        FT_TS_Int     end_point;
        FT_TS_UInt    contour_ix;
        ...
        if ( overlap_bitmap_offset )
        {
          ...
          if ( FT_TS_STREAM_SEEK( overlap_offset ) ||
               FT_TS_READ_BYTE( overlap_bitmap )   )
            goto Fail;
          if ( overlap_bitmap & ( 0x80 >> ( i & 7 ) ) )
            have_overlap = TRUE;
        }

        if ( FT_TS_NEW_ARRAY( n_points_arr, n_contours ) )
          goto Fail;

        if ( FT_TS_STREAM_SEEK( substreams[N_POINTS_STREAM].offset ) )
          goto Fail;
        ...
        substreams[N_POINTS_STREAM].offset = FT_TS_STREAM_POS();
        ...
        if ( FT_TS_NEW_ARRAY( points, points_size ) )
          goto Fail;
      ...
      else
      {
        ...
        if ( have_bbox )
        {
          FT_TS_ERROR(( "Empty glyph has a bbox.\n" ));
          goto Fail;
        }
      }
      ...
      *glyf_checksum += compute_ULong_sum( glyph_buf, glyph_size );
      ...
      if ( n_contours > 0 )
        info->x_mins[i] = (FT_TS_Short)x_min;
    }
    ...
    FT_TS_TRACE4(( "  loca table info:\n" ));
    FT_TS_TRACE4(( "    dst_offset = %lu\n", info->loca_table->dst_offset ));
    FT_TS_TRACE4(( "    dst_length = %lu\n", info->loca_table->dst_length ));
    FT_TS_TRACE4(( "    checksum = %09lx\n", *loca_checksum ));

    /* Set pointer `sfnt_bytes' to its correct value. */
    *sfnt_bytes = sfnt;
    *out_offset = dest_offset;

    FT_TS_FREE( substreams );
    FT_TS_FREE( loca_values );
    FT_TS_FREE( n_points_arr );
    FT_TS_FREE( glyph_buf );
    FT_TS_FREE( points );
  ...
  static FT_TS_Error
  get_x_mins( FT_TS_Stream     stream,
              WOFF2_Table*  tables,
              FT_TS_UShort     num_tables,
              WOFF2_Info    info,
              FT_TS_Memory     memory )
  {
    FT_TS_UShort  num_glyphs;
    FT_TS_UShort  index_format;
    FT_TS_ULong   glyf_offset;
    FT_TS_UShort  glyf_offset_short;
    FT_TS_ULong   loca_offset;
    FT_TS_Int     i;
    FT_TS_Error   error = FT_TS_Err_Ok;
    FT_TS_ULong   offset_size;
    ...
    if ( !maxp_table )
    {
      FT_TS_ERROR(( "`maxp' table is missing.\n" ));
      return FT_TS_THROW( Invalid_Table );
    }

    if ( !head_table )
    {
      FT_TS_ERROR(( "`head' table is missing.\n" ));
      return FT_TS_THROW( Invalid_Table );
    }

    if ( !info->loca_table )
    {
      FT_TS_ERROR(( "`loca' table is missing.\n" ));
      return FT_TS_THROW( Invalid_Table );
    }

    /* Read `numGlyphs' field from `maxp' table. */
    if ( FT_TS_STREAM_SEEK( maxp_table->src_offset ) || FT_TS_STREAM_SKIP( 8 ) )
      return error;
    ...
    if ( FT_TS_STREAM_SEEK( head_table->src_offset ) ||
         FT_TS_STREAM_SKIP( 50 )                     )
      return error;
    ...
    for ( i = 0; i < num_glyphs; ++i )
    {
      if ( FT_TS_STREAM_SEEK( loca_offset ) )
        return error;
      ...
      if ( index_format )
      {
        if ( FT_TS_READ_ULONG( glyf_offset ) )
          return error;
      }
      else
      {
        if ( FT_TS_READ_USHORT( glyf_offset_short ) )
          return error;

        glyf_offset = (FT_TS_ULong)( glyf_offset_short );
        glyf_offset = glyf_offset << 1;
      }

      glyf_offset += info->glyf_table->src_offset;

      if ( FT_TS_STREAM_SEEK( glyf_offset ) || FT_TS_STREAM_SKIP( 2 ) )
        return error;

      if ( FT_TS_READ_SHORT( info->x_mins[i] ) )
        return error;
    }
  ...
  static FT_TS_Error
  reconstruct_hmtx( FT_TS_Stream  stream,
                    FT_TS_UShort  num_glyphs,
                    FT_TS_UShort  num_hmetrics,
                    FT_TS_Short*  x_mins,
                    FT_TS_ULong*  checksum,
                    ...
                    FT_TS_Memory  memory )
  {
    ...
    FT_TS_Byte   hmtx_flags;
    FT_TS_Bool   has_proportional_lsbs, has_monospace_lsbs;
    FT_TS_ULong  hmtx_table_size;
    FT_TS_Int    i;

    FT_TS_UShort*  advance_widths = NULL;
    FT_TS_Short*   lsbs           = NULL;
    FT_TS_Byte*    hmtx_table     = NULL;
    FT_TS_Byte*    dst            = NULL;


    if ( FT_TS_READ_BYTE( hmtx_flags ) )
      goto Fail;
    ...
    if ( FT_TS_NEW_ARRAY( advance_widths, num_hmetrics ) ||
         FT_TS_NEW_ARRAY( lsbs, num_glyphs )             )
      goto Fail;

    /* Read `advanceWidth' stream.  Always present. */
    for ( i = 0; i < num_hmetrics; i++ )
    {
      FT_TS_UShort  advance_width;


      if ( FT_TS_READ_USHORT( advance_width ) )
        goto Fail;
    ...
    if ( FT_TS_NEW_ARRAY( hmtx_table, hmtx_table_size ) )
      goto Fail;

    dst = hmtx_table;
    FT_TS_TRACE6(( "hmtx values: \n" ));
    for ( i = 0; i < num_glyphs; i++ )
    {
      if ( i < num_hmetrics )
      {
        WRITE_SHORT( dst, advance_widths[i] );
        FT_TS_TRACE6(( "%d ", advance_widths[i] ));
      }

      WRITE_SHORT( dst, lsbs[i] );
      FT_TS_TRACE6(( "%d ", lsbs[i] ));
    }
    FT_TS_TRACE6(( "\n" ));
    ...
    *out_offset = dest_offset;
    ...
    FT_TS_FREE( advance_widths );
    FT_TS_FREE( lsbs );
    FT_TS_FREE( hmtx_table );
  ...
  static FT_TS_Error
  reconstruct_font( FT_TS_Byte*      transformed_buf,
                    FT_TS_ULong      transformed_buf_size,
                    WOFF2_Table*  indices,
                    WOFF2_Header  woff2,
                    WOFF2_Info    info,
                    FT_TS_Byte**     sfnt_bytes,
                    ...
                    FT_TS_Memory     memory )
  {
    ...
    FT_TS_Byte*   buf_cursor  = NULL;
    FT_TS_Byte*   table_entry = NULL;

    /* We are reallocating memory for `sfnt', so its pointer may change. */
    FT_TS_Byte*   sfnt = *sfnt_bytes;

    FT_TS_UShort  num_tables  = woff2->num_tables;
    FT_TS_ULong   dest_offset = 12 + num_tables * 16UL;

    FT_TS_ULong   checksum      = 0;
    FT_TS_ULong   loca_checksum = 0;
    FT_TS_Int     nn            = 0;
    FT_TS_UShort  num_hmetrics  = 0;
    FT_TS_ULong   font_checksum = info->header_checksum;
    FT_TS_Bool    is_glyf_xform = FALSE;

    FT_TS_ULong  table_entry_offset = 12;
    ...
    {
      FT_TS_ERROR(( "One of `glyf'/`loca' tables missing.\n" ));
      return FT_TS_THROW( Invalid_Table );
    }
    ...
    if ( info->glyf_table != NULL )
    {
      ...
      {
        FT_TS_ERROR(( "Transformation mismatch"
    ...
    if ( FT_TS_NEW_ARRAY( table_entry, 16 ) )
      goto Fail;

    /* Create a stream for the uncompressed buffer. */
    if ( FT_TS_NEW( stream ) )
      goto Fail;
    FT_TS_Stream_OpenMemory( stream, transformed_buf, transformed_buf_size );

    FT_TS_ASSERT( FT_TS_STREAM_POS() == 0 );
    ...
    for ( nn = 0; nn < num_tables; nn++ )
    {
      ...
      FT_TS_TRACE3(( "Seeking to %ld with table size %ld.\n",
                  table.src_offset, table.src_length ));
      FT_TS_TRACE3(( "Table tag: %c%c%c%c.\n",
                  (FT_TS_Char)( table.Tag >> 24 ),
                  (FT_TS_Char)( table.Tag >> 16 ),
                  (FT_TS_Char)( table.Tag >> 8  ),
                  (FT_TS_Char)( table.Tag       ) ));

      if ( FT_TS_STREAM_SEEK( table.src_offset ) )
        goto Fail;
      ...
      {
        FT_TS_TRACE3(( "This table is transformed.\n" ));
        ...
        {
          /* Unknown transform. */
          FT_TS_ERROR(( "Unknown table transform.\n" ));
          goto Fail;
        }
      }
      ...
      /* Sanity check. */
      if ( (FT_TS_ULong)( table.dst_offset + table.dst_length ) > dest_offset )
      {
        FT_TS_ERROR(( "Table was partially written.\n" ));
        goto Fail;
      }
    }
    ...
    FT_TS_TRACE2(( "Final checksum = %09lx.\n", font_checksum ));
                   ...
                   FT_TS_Long*   num_faces )
  {
    ...
    FT_TS_ULong   j;
    FT_TS_ULong   flags;
    FT_TS_UShort  xform_version;
    FT_TS_ULong   src_offset = 0;

    FT_TS_UInt    glyf_index;
    FT_TS_UInt    loca_index;
    FT_TS_UInt32  file_offset;

    FT_TS_Byte*   sfnt        = NULL;
    FT_TS_Stream  sfnt_stream = NULL;
    FT_TS_Byte*   sfnt_header;
    FT_TS_ULong   sfnt_size;

    FT_TS_Byte*  uncompressed_buf = NULL;

    static const FT_TS_Frame_Field  woff2_header_fields[] =
    {
#undef  FT_TS_STRUCTURE
#define FT_TS_STRUCTURE  WOFF2_HeaderRec

      FT_TS_FRAME_START( 48 ),
        FT_TS_FRAME_ULONG     ( signature ),
        FT_TS_FRAME_ULONG     ( flavor ),
        FT_TS_FRAME_ULONG     ( length ),
        FT_TS_FRAME_USHORT    ( num_tables ),
        FT_TS_FRAME_SKIP_BYTES( 2 ),
        FT_TS_FRAME_ULONG     ( totalSfntSize ),
        FT_TS_FRAME_ULONG     ( totalCompressedSize ),
        FT_TS_FRAME_SKIP_BYTES( 2 * 2 ),
        FT_TS_FRAME_ULONG     ( metaOffset ),
        FT_TS_FRAME_ULONG     ( metaLength ),
        FT_TS_FRAME_ULONG     ( metaOrigLength ),
        FT_TS_FRAME_ULONG     ( privOffset ),
        FT_TS_FRAME_ULONG     ( privLength ),
      FT_TS_FRAME_END
    };


    FT_TS_ASSERT( stream == face->root.stream );
    FT_TS_ASSERT( FT_TS_STREAM_POS() == 0 );

    face_index = FT_TS_ABS( *face_instance_index ) & 0xFFFF;

    /* Read WOFF2 Header. */
    if ( FT_TS_STREAM_READ_FIELDS( woff2_header_fields, &woff2 ) )
      return error;

    FT_TS_TRACE4(( "signature     -> 0x%lX\n", woff2.signature ));
    FT_TS_TRACE2(( "flavor        -> 0x%08lx\n", woff2.flavor ));
    FT_TS_TRACE4(( "length        -> %lu\n", woff2.length ));
    FT_TS_TRACE2(( "num_tables    -> %hu\n", woff2.num_tables ));
    FT_TS_TRACE4(( "totalSfntSize -> %lu\n", woff2.totalSfntSize ));
    FT_TS_TRACE4(( "metaOffset    -> %lu\n", woff2.metaOffset ));
    FT_TS_TRACE4(( "metaLength    -> %lu\n", woff2.metaLength ));
    FT_TS_TRACE4(( "privOffset    -> %lu\n", woff2.privOffset ));
    FT_TS_TRACE4(( "privLength    -> %lu\n", woff2.privLength ));
    ...
    {
      FT_TS_ERROR(( "woff2_open_font: invalid WOFF2 header\n" ));
      return FT_TS_THROW( Invalid_Table );
    }

    FT_TS_TRACE2(( "woff2_open_font: WOFF2 Header is valid.\n" ));

    woff2.ttc_fonts = NULL;

    /* Read table directory. */
    if ( FT_TS_NEW_ARRAY( tables, woff2.num_tables )  ||
         FT_TS_NEW_ARRAY( indices, woff2.num_tables ) )
      goto Exit;

    FT_TS_TRACE2(( "\n" ));
    FT_TS_TRACE2(( "  tag    flags    transform  origLen   transformLen   offset\n" ));
    FT_TS_TRACE2(( "  -----------------------------------------------------------\n" ));
    ...
    for ( nn = 0; nn < woff2.num_tables; nn++ )
    {
      ...
      if ( FT_TS_READ_BYTE( table->FlagByte ) )
        goto Exit;

      if ( ( table->FlagByte & 0x3f ) == 0x3f )
      {
        if ( FT_TS_READ_ULONG( table->Tag ) )
          goto Exit;
      }
      else
      {
        ...
        if ( !table->Tag )
        {
          FT_TS_ERROR(( "woff2_open_font: Unknown table tag." ));
          error = FT_TS_THROW( Invalid_Table );
          goto Exit;
        }
      }
      ...
      if ( ( flags & WOFF2_FLAGS_TRANSFORM ) != 0 )
      {
        ...
        {
          FT_TS_ERROR(( "woff2_open_font: Invalid loca `transformLength'.\n" ));
          error = FT_TS_THROW( Invalid_Table );
          goto Exit;
        }
      }
      ...
      {
        FT_TS_ERROR(( "woff2_open_font: invalid WOFF2 table directory.\n" ));
        error = FT_TS_THROW( Invalid_Table );
        goto Exit;
      }
      ...
      FT_TS_TRACE2(( "  %c%c%c%c  %08d  %08d   %08ld    %08ld    %08ld\n",
                  (FT_TS_Char)( table->Tag >> 24 ),
                  (FT_TS_Char)( table->Tag >> 16 ),
                  (FT_TS_Char)( table->Tag >> 8  ),
                  (FT_TS_Char)( table->Tag       ),
    ...
    FT_TS_TRACE2(( "Table directory parsed.\n" ));
    ...
    {
      FT_TS_TRACE2(( "Font is a TTC, reading collection directory.\n" ));

      if ( FT_TS_READ_ULONG( woff2.header_version ) )
        goto Exit;
      ...
      FT_TS_TRACE4(( "Number of fonts in TTC: %d\n", woff2.num_fonts ));

      if ( FT_TS_NEW_ARRAY( woff2.ttc_fonts, woff2.num_fonts ) )
        goto Exit;
      ...
      for ( nn = 0; nn < woff2.num_fonts; nn++ )
      {
        ...
        if ( READ_255USHORT( ttc_font->num_tables ) )
          goto Exit;
        if ( FT_TS_READ_ULONG( ttc_font->flavor ) )
          goto Exit;

        if ( FT_TS_NEW_ARRAY( ttc_font->table_indices, ttc_font->num_tables ) )
          goto Exit;

        FT_TS_TRACE5(( "Number of tables in font %d: %d\n",
                    nn, ttc_font->num_tables ));

#ifdef FT_TS_DEBUG_LEVEL_TRACE
        if ( ttc_font->num_tables )
          FT_TS_TRACE6(( "  Indices: " ));
#endif
        ...
        for ( j = 0; j < ttc_font->num_tables; j++ )
        {
          ...
          FT_TS_TRACE6(( "%hu ", table_index ));
          if ( table_index >= woff2.num_tables )
          {
            FT_TS_ERROR(( "woff2_open_font: invalid table index\n" ));
            error = FT_TS_THROW( Invalid_Table );
            goto Exit;
          }
      ...
      FT_TS_TRACE2(( "WOFF2 collection directory is valid.\n" ));
    }
    else
      woff2.ttc_fonts = NULL;

    woff2.compressed_offset = FT_TS_STREAM_POS();
    ...
    *num_faces = woff2.num_fonts;
    ...
    if ( woff2.totalSfntSize > sfnt_size )
    {
      ...
      if ( sfnt_size != woff2.totalSfntSize )
        FT_TS_TRACE4(( "adjusting estimate of uncompressed font size"
                    " to %lu bytes\n",
                    sfnt_size ));
#endif
    }
    ...
    /* Write sfnt header. */
    if ( FT_TS_QALLOC( sfnt, sfnt_size ) ||
         FT_TS_NEW( sfnt_stream )        )
      goto Exit;
    ...
    if ( woff2.num_tables )
    {
      FT_TS_UInt  searchRange, entrySelector, rangeShift, x;
    ...
    {
      FT_TS_Tag  tag = indices[nn]->Tag;


      if ( tag == indices[nn - 1]->Tag )
      {
        FT_TS_ERROR(( "woff2_open_font:"
    ...
    {
      FT_TS_ERROR(( "woff2_open_font: SFNT table lengths are too large.\n" ));
      error = FT_TS_THROW( Invalid_Table );
      goto Exit;
    }

    /* Allocate memory for uncompressed table data. */
    if ( FT_TS_QALLOC( uncompressed_buf, woff2.uncompressed_size ) ||
         FT_TS_FRAME_ENTER( woff2.totalCompressedSize )            )
      goto Exit;
    ...
    {
      FT_TS_TRACE5(( "Trimming sfnt stream from %lu to %lu.\n",
                  sfnt_size, woff2.actual_sfnt_size ));
      if ( FT_TS_REALLOC( sfnt,
                       (FT_TS_ULong)( sfnt_size ),
                       (FT_TS_ULong)( woff2.actual_sfnt_size ) ) )
        goto Exit;
    }

    /* `reconstruct_font' has done all the work. */
    /* Swap out stream and return.               */
    FT_TS_Stream_OpenMemory( sfnt_stream, sfnt, woff2.actual_sfnt_size );
    sfnt_stream->memory = stream->memory;
    sfnt_stream->close  = stream_close;

    FT_TS_Stream_Free(
      face->root.stream,
      ( face->root.face_flags & FT_TS_FACE_FLAG_EXTERNAL_STREAM ) != 0 );

    face->root.stream      = sfnt_stream;
    face->root.face_flags &= ~FT_TS_FACE_FLAG_EXTERNAL_STREAM;
    ...
    FT_TS_TRACE2(( "woff2_open_font: SFNT synthesized.\n" ));

  Exit:
    FT_TS_FREE( tables );
    FT_TS_FREE( indices );
    FT_TS_FREE( uncompressed_buf );
    FT_TS_FREE( info.x_mins );
    ...
    if ( woff2.ttc_fonts )
    {
      ...
      for ( nn = 0; nn < woff2.num_fonts; nn++ )
      {
        FT_TS_FREE( ttc_font->table_indices );
        ttc_font++;
      }

      FT_TS_FREE( woff2.ttc_fonts );
    }
    ...
    if ( error )
    {
      ...
      if ( sfnt_stream )
      {
        FT_TS_Stream_Close( sfnt_stream );
        FT_TS_FREE( sfnt_stream );
      }
    }
