...
#define FT_TS_COMPONENT  t1objs
  ...
  T1_Size_Done( FT_TS_Size  t1size )          /* T1_Size */
  {
  ...
  T1_Size_Init( FT_TS_Size  t1size )      /* T1_Size */
  {
    T1_Size            size  = (T1_Size)t1size;
  ...
  FT_TS_LOCAL_DEF( FT_TS_Error )
  T1_Size_Request( FT_TS_Size          t1size,     /* T1_Size */
                   FT_TS_Size_Request  req )
  {
    ...
    error = FT_TS_Request_Metrics( size->root.face, req );
    if ( error )
      goto Exit;
   ...
   */
  FT_TS_LOCAL_DEF( void )
  T1_Face_Done( FT_TS_Face  t1face )         /* T1_Face */
  {
    ...
    FT_TS_FREE( type1->subrs );
    FT_TS_FREE( type1->subrs_len );

    ft_hash_num_free( type1->subrs_hash, memory );
    FT_TS_FREE( type1->subrs_hash );

    FT_TS_FREE( type1->subrs_block );
...
#if 0
    FT_TS_FREE( face->unicode_map_rec.maps );
                ...
                FT_TS_Parameter*  params )
  {
    ...
    if ( !psaux )
    {
      FT_TS_ERROR(( "T1_Face_Init: cannot access `psaux' module\n" ));
      error = FT_TS_THROW( Missing_Module );
      goto Exit;
    }

    face->pshinter = FT_TS_Get_Module_Interface( FT_TS_FACE_LIBRARY( face ),
                                              "pshinter" );

    FT_TS_TRACE2(( "Type 1 driver\n" ));
    ...
    FT_TS_TRACE2(( "T1_Face_Init: %p (index %d)\n",
                (void *)face,
                face_index ));
    ...
    {
      FT_TS_ERROR(( "T1_Face_Init: invalid face index\n" ));
    ...
    /* set up root face fields */
    {
      ...
      if ( face->blend )
        root->face_flags |= FT_TS_FACE_FLAG_MULTIPLE_MASTERS;
      ...
      root->ascender  = (FT_TS_Short)( root->bbox.yMax );
      root->descender = (FT_TS_Short)( root->bbox.yMin );

      root->height = (FT_TS_Short)( ( root->units_per_EM * 12 ) / 10 );
      if ( root->height < root->ascender - root->descender )
        root->height = (FT_TS_Short)( root->ascender - root->descender );

      /* now compute the maximum advance width */
      root->max_advance_width =
        (FT_TS_Short)( root->bbox.xMax );
      {
        ...
        if ( !error )
          root->max_advance_width = (FT_TS_Short)FIXED_TO_INT( max_advance );
        else
          error = FT_TS_Err_Ok;   /* clear error */
      }
   ...
   */
  FT_TS_LOCAL_DEF( FT_TS_Error )
  T1_Driver_Init( FT_TS_Module  module )
  {
