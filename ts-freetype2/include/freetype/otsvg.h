...
FT_TS_BEGIN_HEADER
   ...
   *   the lifetime of an @FT_TS_Library object.  In a typical implementation,
   ...
   */
  typedef FT_TS_Error
  (*SVG_Lib_Init_Func)( FT_TS_Pointer  *data_pointer );
   ...
   */
  typedef void
  (*SVG_Lib_Free_Func)( FT_TS_Pointer  *data_pointer );
   ...
   *   render is available through the handle @FT_TS_SVG_Document, which is set
   *   in the `other` field of @FT_TS_GlyphSlotRec.
   *
   ...
   */
  typedef FT_TS_Error
  (*SVG_Lib_Render_Func)( FT_TS_GlyphSlot  slot,
                          FT_TS_Pointer   *data_pointer );
   ...
   *   two places.
   *
   *   1. When `FT_TS_Load_Glyph` needs to preset the glyph slot.
   *
   ...
   */
  typedef FT_TS_Error
  (*SVG_Lib_Preset_Slot_Func)( FT_TS_GlyphSlot  slot,
                               FT_TS_Bool       cache,
                               FT_TS_Pointer   *state );
   ...
   *
   * @note:
   *   When an @FT_TS_GlyphSlot object `slot` is passed down to a renderer, the
   *   renderer can only access the `metrics` and `units_per_EM` fields via
   *   `slot->face`.  However, when @FT_TS_Glyph_To_Bitmap sets up a dummy
   ...
   *   2.12
   */
  typedef struct  FT_TS_SVG_DocumentRec_
  {
   ...
   *
   * @description:
   *   A handle to an @FT_TS_SVG_DocumentRec object.
   *
   * @since:
   *   2.12
   */
  typedef struct FT_TS_SVG_DocumentRec_*  FT_TS_SVG_Document;
