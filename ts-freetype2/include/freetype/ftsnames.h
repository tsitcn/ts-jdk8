...
FT_TS_BEGIN_HEADER
   ...
   *     function @FT_TS_Get_Sfnt_LangTag with `language_id` as its argument to
   ...
   *   details.
   */
  typedef struct  FT_TS_SfntName_
  {
    FT_TS_UShort  platform_id;
    FT_TS_UShort  encoding_id;
    FT_TS_UShort  language_id;
    FT_TS_UShort  name_id;

    FT_TS_Byte*   string;      /* this string is *not* null-terminated! */
    FT_TS_UInt    string_len;  /* in bytes                              */
   ...
   * @output:
   *   aname ::
   *     The indexed @FT_TS_SfntName structure.
   *
   * @return:
   ...
   *   yourself; FreeType takes care of it if you call @FT_TS_Done_Face.
   *
   *   Use @FT_TS_Get_Sfnt_Name_Count to get the total number of available
   *   'name' table entries, then do a loop until you get the right platform,
   *   encoding, and name ID.
   *
   *   'name' table format~1 entries can use language tags also, see
   *   @FT_TS_Get_Sfnt_LangTag.
   *
   *   This function always returns an error if the config macro
   *   `TT_CONFIG_OPTION_SFNT_NAMES` is not defined in `ftoption.h`.
   */
   ...
   *   2.8
   */
  typedef struct  FT_TS_SfntLangTag_
  {
    FT_TS_Byte*  string;      /* this string is *not* null-terminated! */
    FT_TS_UInt   string_len;  /* in bytes                              */
   ...
   *
   *   langID ::
   *     The language ID, as returned by @FT_TS_Get_Sfnt_Name.  This is always a
   ...
   *   tables, this function always returns FT_TS_Err_Invalid_Table.  For
   *   invalid format~1 language ID values, FT_TS_Err_Invalid_Argument is
   *   returned.
   *
   *   This function always returns an error if the config macro
   *   `TT_CONFIG_OPTION_SFNT_NAMES` is not defined in `ftoption.h`.
