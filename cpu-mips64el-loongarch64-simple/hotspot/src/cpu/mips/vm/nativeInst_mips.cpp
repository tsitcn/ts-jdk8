...
#else
  long dest = ((long)dst - (((long)addr_at(4)) & 0xf0000000))>>2;
#endif
  if ((dest >= 0) && (dest < (1<<26))) {
...
address NativeJump::jump_destination() {
  ...
  // 0x92eea8:    bal    0x92eeb0
  // 0x92eeac:    lui    t9,0x3
  // 0x92eeb0:    ori    t9,t9,0x8f44
  // 0x92eeb4:    daddu  t9,t9,ra
  // 0x92eeb8:    jr     t9
  // 0x92eebc:    nop
  if (  is_op(int_at(4), Assembler::lui_op)  &&
        is_op (int_at(8), Assembler::ori_op) &&
        is_special_op(int_at(16), Assembler::jr_op) ) {
    address target = addr_at(4) + Assembler::imm_off(int_at(instruction_offset)) * 4;

    int sign = int_at(0) & 0x8000;
    if (sign == 0) {
      return target + Assembler::merge( (intptr_t)(int_at(4) & 0xffff),
                                        (intptr_t)(int_at(0) & 0xffff),
                                        (intptr_t)0,
                                        (intptr_t)0);
    } else {
